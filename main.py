...
It first sets up logging, then loads the version hash from version.ini (if it exists), then loads the cats and pack.
...
if os.path.exists("auto-updated"):
    print("Wolfgen starting, deleting auto-updated file")
...
# Logging for file
timestr = time.strftime("%Y%m%d_%H%M%S")
log_file_name = get_log_dir() + f"/wolfgen_{timestr}.log"
...
if os.environ.get('CODESPACES'):
    ...
    print(
        ...
        + "/?autoconnect=true&reconnect=true&password=wolfgen&resize=scale")
    print("(use wolfgen in fullscreen mode for best results)")
    print('')
...
from scripts.pack import pack_class
...
# LOAD cats & pack
pack_list = game.read_packs()
if pack_list:
    game.switches['pack_list'] = pack_list
    try:
        load_cats()
        version_info = pack_class.load_pack()
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            # Dont display if on the start screen or there is no pack.
