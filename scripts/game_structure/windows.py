...
class SaveCheck(UIWindow):
    def __init__(self, last_screen, isMainMenu, mm_btn):
        ...
        self.pack_name = "UndefinedPack"
        if game.pack:
            self.pack_name = f"{game.pack.name}Pack"
...
class DeleteCheck(UIWindow):
    def __init__(self, reloadscreen, pack_name):
        ...
        self.delete_check_message = UITextBoxTweaked(
            f"Do you wish to delete {str(self.pack_name + 'Pack')}? This is permanent and cannot be undone.",
            scale(pygame.Rect((40, 40), (520, -1))),
            line_spacing=1,
            object_id="#text_box_30_horizcenter",
            container=self
    ...
    def process_event(self, event):
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.delete_it_button:
                game.switches['window_open'] = False
                print("delete")
                rempath = get_save_dir() + "/" + self.pack_name
                shutil.rmtree(rempath)
                if os.path.exists(rempath + 'pack.json'):
                    os.remove(rempath + "pack.json")
                elif os.path.exists(rempath + 'pack.txt'):
                    os.remove(rempath + "pack.txt")
                else:
                    print("No pack.json/txt???? pack prolly wasnt initalized kekw")
                self.kill()
                self.reloadscreen('switch pack screen')
...
class GameOver(UIWindow):
    def __init__(self, last_screen):
        ...
        self.set_blocking(True)
        game.switches['window_open'] = True
        self.pack_name = str(game.pack.name + 'pack')
        self.last_screen = last_screen
        self.game_over_message = UITextBoxTweaked(
            f"{self.pack_name} has died out. For now, this is where their story ends. Perhaps it's time to tell a new "
            f"tale?",
            scale(pygame.Rect((40, 40), (520, -1))),
            line_spacing=1,
            object_id="",
...
class KillCat(UIWindow):
    ...
    def __init__(self, cat):
        ...
        if self.the_cat.status == 'leader':
            ...
            self.initial= process_text('{VERB/m_c/were/was} killed by something unknowable to even StarPack', cat_dict)
            ...
            self.initial = 'It was the will of something even mightier than StarPack that this cat died.'
            self.prompt = None
            self.all_lives_check.hide()
            self.one_life_check.hide()
...
class UpdateAvailablePopup(UIWindow):
    def __init__(self, last_screen, show_checkbox: bool = False):
        ...
        self.game_over_message = UITextBoxTweaked(
            f"<strong>Update to PackGen {latest_version_number}</strong>",
