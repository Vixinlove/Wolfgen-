...
def load_cats():
    try:
        ...
        try:
            ...
            game.switches['error_message'] = 'Can\'t find pack_cats.json!'
...
def json_load():
    ...
    pack_cats_json_path = f'{get_save_dir()}/{packname}/pack_cats.json'
    with open(f"resources/dicts/conversion_dict.json", 'r') as read_file:
        convert = ujson.loads(read_file.read())
    try:
        with open(pack_cats_json_path, 'r') as read_file:
            cat_data = ujson.loads(read_file.read())
    except PermissionError as e:
        game.switches['error_message'] = f'Can\t open {pack_cats_json_path}!'
        game.switches['traceback'] = e
        raise
    except ujson.JSONDecodeError as e:
        game.switches['error_message'] = f'{pack_cats_json_path} is malformed!'
        game.switches['traceback'] = e
        raise
    ...
    for i, cat in enumerate(cat_data):
        try:
            ...
            if "ID" in cat:
                ...
                key = f" at index {i} "
            game.switches['error_message'] = f'Cat{key}in pack_cats.json is missing {e}!'
            game.switches['traceback'] = e
            raise
...
def csv_load(all_cats):
    if game.switches['pack_list'][0].strip() == '':
        ...
        if os.path.exists(get_save_dir() + '/' + game.switches['pack_list'][0] +
                          'cats.csv'):
            with open(get_save_dir() + '/' + game.switches['pack_list'][0] + 'cats.csv',
                      'r') as read_file:
                cat_data = read_file.read()
        else:
            with open(get_save_dir() + '/' + game.switches['pack_list'][0] + 'cats.txt',
                      'r') as read_file:
                cat_data = read_file.read()
    if len(cat_data) > 0:
        ...
        game.switches[
            'error_message'] = 'There was an error loading this pack\'s mentors, apprentices, relationships, or sprite info.'
        for inter_cat in all_cats.values():
            # Load the mentors and apprentices after all cats have been loaded
            game.switches[
                'error_message'] = 'There was an error loading this pack\'s mentors/apprentices. Last cat read was ' + str(
                inter_cat)
            ...
            if not inter_cat.dead:
                game.switches[
                    'error_message'] = 'There was an error loading this pack\'s relationships. Last cat read was ' + str(
                    inter_cat)
...
def version_convert(version_info):
    ...
    This is a separate function, since the version info is stored in pack.json, but most conversion needs to be
    ...
    if version < 2:
        for c in Cat.all_cats.values():
            for con in c.injuries:
                ...
                c.injuries[con]["moon_start"] = game.pack.age - moons_with
            ...
            for con in c.illnesses:
                ...
                c.illnesses[con]["moon_start"] = game.pack.age - moons_with
            ...
            for con in c.permanent_condition:
                ...
                c.permanent_condition[con]["moon_start"] = game.pack.age - moons_with
