...
class Game():
    ...
    def read_packs(self):
        '''with open(get_save_dir() + '/packlist.txt', 'r') as read_file:
            pack_list = read_file.read()
            if_packs = len(pack_list)
        if if_packs > 0:
            pack_list = pack_list.split('\n')
            pack_list = [i.strip() for i in pack_list if i]  # Remove empty and whitespace
            return pack_list
        else:
            return None'''
        # All of the above is old code
        # Now, we want packlist.txt to contain ONLY the name of the pack that is currently loaded
        # We will get the list of packs from the saves folder
        # each pack has its own folder, and the name of the folder is the name of the pack
        ...
        pack_list = [f.name for f in os.scandir(get_save_dir()) if f.is_dir()]

        # the pack specified in saves/packlist.txt should be first in the list
        # so we can load it automatically

        if os.path.exists(get_save_dir() + '/packlist.txt'):
            with open(get_save_dir() + '/packlist.txt', 'r') as f:
                loaded_pack = f.read().strip().splitlines()
                if loaded_pack:
                    loaded_pack = loaded_pack[0]
                else:
                    loaded_pack = None
            os.remove(get_save_dir() + '/packlist.txt')
            if loaded_pack:
                self.safe_save(get_save_dir() + '/currentpack.txt', loaded_pack)
        elif os.path.exists(get_save_dir() + '/currentpack.txt'):
            with open(get_save_dir() + '/currentpack.txt', 'r') as f:
                loaded_pack = f.read().strip()
        else:
            loaded_pack = None

        if loaded_pack and loaded_pack in pack_list:
            pack_list.remove(loaded_pack)
            pack_list.insert(0, loaded_pack)

        # Now we can return the list of packs
        if not pack_list:
            print('No packs found')
            return None
        # print('packs found:', pack_list)
        return pack_list

    def save_packlist(self, loaded_pack=None):
        '''packs = []
        if loaded_pack:
            packs.append(f"{loaded_pack}\n")

        for pack_name in self.switches['pack_list']:
            if pack_name and pack_name != loaded_pack:
                packs.append(f"{pack_name}\n")

        if packs:
            with open(get_save_dir() + '/packlist.txt', 'w') as f:
                f.writelines(packs)'''
        if loaded_pack:
            if os.path.exists(get_save_dir() + '/packlist.txt'):
                os.remove(get_save_dir() + '/packlist.txt')  # we don't need packlist.txt anymore
            game.safe_save(f"{get_save_dir()}/currentpack.txt", loaded_pack)
        else:
            if os.path.exists(get_save_dir() + '/currentpack.txt'):
                os.remove(get_save_dir() + '/currentpack.txt')
    ...
    def save_cats(self):
        ...
        directory = get_save_dir() + '/' + packname
        ...
        self.save_faded_cats(packname)  # Fades cat and saves them, if needed

        pack_cats = []
        for inter_cat in self.cat_class.all_cats.values():
            cat_data = inter_cat.get_save_dict()
            pack_cats.append(cat_data)
        ...
        self.safe_save(f"{get_save_dir()}/{packname}/pack_cats.json", pack_cats)

    def save_faded_cats(self, packname):
        """Deals with fades cats, if needed, adding them as faded """
        if game.cat_to_fade:
            directory = get_save_dir() + '/' + packname + "/faded_cats"
            if not os.path.exists(directory):
                os.makedirs(directory)
        ...
        for cat in game.cat_to_fade:
            ...
            self.pack.faded_ids.append(cat)
            ...
            self.safe_save(f"{get_save_dir()}/{packname}/faded_cats/{cat}.json", cat_data)

            self.pack.remove_cat(cat)  # Remove the cat from the active cats lists

        game.cat_to_fade = []
        
        # Save the copies, flush the file. 
        if game.settings["save_faded_copy"]:
            with open(get_save_dir() + '/' + packname + '/faded_cats_info_copy.txt', 'a') as write_file:
                
                if not os.path.exists(get_save_dir() + '/' + packname + '/faded_cats_info_copy.txt'):
                    # Create the file if it doesn't exist
                    with open(get_save_dir() + '/' + packname + '/faded_cats_info_copy.txt', 'w') as create_file:
                        pass
    ...
    def save_events(self):
        ...
        game.safe_save(f"{get_save_dir()}/{game.pack.name}/events.json", events_list)
    ...
    def add_faded_offspring_to_faded_cat(self, parent, offspring):
        ...
        try:
            with open(get_save_dir() + '/' + self.pack.name + '/faded_cats/' + parent + ".json", 'r') as read_file:
        ...
        self.safe_save(f"{get_save_dir()}/{self.pack.name}/faded_cats/{parent}.json", cat_info)
    ...
    def load_events(self):
        ...
        packname = self.pack.name
        events_path = f'{get_save_dir()}/{packname}/events.json'
        events_list = []
        try:
    ...
    def get_config_value(self, *args):
        ...
        This function will apply war modifers if the pack is currently at war. """
        ...
        # Apply war if needed
        if self.pack and self.pack.war.get("at_war", False) and args in war_effected:
            # Grabs the modifer
            mod = self.config
