...
status_dict = {
    ...
    "make pack screen": "Making a Pack",
    ...
    "starpack screen": "Viewing StarPack",
    "dark forest screen": "Viewing the Place of no Return",
...
class _DiscordRPC(threading.Thread):
    ...
    def update(self):
        if self._connected:
            ...
            try:
                img_str = f"{game.pack.biome}_{game.pack.current_season.replace('-', '')}_{game.pack.camp_bg}_{'dark' if game.settings['dark mode'] else 'light'}"
                img_text = game.pack.biome
            ...
            cats_amount = 0
            if game.pack:
                pack_name =  f"{game.pack.name}Pack"
                cats_amount = len(game.pack.pack_cats)
                pack_age = game.pack.age
            else:
                pack_name = 'Loading...'
                cats_amount = 0
                pack_age = 0
            try:
                self._rpc.update(
                    state=state_text,
                    details=f"Managing {pack_name} for {pack_age} moons" ,
                    large_image=img_str.lower(),
                    large_text=img_text,
                    small_image="discord",
                    small_text=f"Managing {cats_amount} cats",
                    start=self._start_time,
                    buttons=[{"label": "Join The Server", "url": "https://discord.gg/packgen"}],
                )
