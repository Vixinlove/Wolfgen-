...
from scripts.pack import HERBS
from scripts.pack_resources.freshkill import FRESHKILL_ACTIVE, FRESHKILL_EVENT_ACTIVE
...
class Events:
    ...
    def one_moon(self):
        ...
        game.mediated = []
        game.switches['saved_pack'] = False
        self.new_cat_invited = False
        ...
        # age up the pack, set current season
        game.pack.age += 1
        get_current_season()
        # print(game.pack.current_season)
        self.pregnancy_events.handle_pregnancy_age(game.pack)
        self.check_war()

        if game.pack.game_mode in ['expanded', 'cruel season'
                                   ] and game.pack.freshkill_pile:
            needed_amount = game.pack.freshkill_pile.amount_food_needed()
            if FRESHKILL_ACTIVE:
                print(f" -- FRESHKILL: prey amount before feeding {game.pack.freshkill_pile.total_amount}") # pylint: disable=line-too-long
            ...
            game.pack.freshkill_pile.time_skip(relevant_cats)
            ...
            if game.pack.age >= 5:
                self.freshkill_events.handle_amount_freshkill_pile(
                    game.pack.freshkill_pile, relevant_cats)
            # make a notification if the pack has not enough prey
            if not game.pack.freshkill_pile.pack_has_enough_food(
            ) and FRESHKILL_EVENT_ACTIVE:
                game.cur_events_list.insert(
                    0,
                    Single_Event(
                        f"{game.pack.name}pack doesn't have enough prey for next moon!"
                    ))
            if FRESHKILL_ACTIVE:
                print(f" -- FRESHKILL: prey amount after feeding {game.pack.freshkill_pile.total_amount}")
        ...
        if Cat.dead_cats:
            ...
            if len(Cat.dead_cats) > 1 and game.pack.game_mode != 'classic':
                event = f"The past moon, {insert} have taken their place in Starpack. {game.pack.name}pack mourns their " \
                ...
                if len(ghost_names) > 2:
                    ...
                    if len(shaken_cats) == 1:
                        extra_event = f"So much grief and death has taken its toll on the cats of {game.pack.name}pack. {insert} is particularly shaken by it."
                    else:
                        extra_event = f"So much grief and death has taken its toll on the cats of {game.pack.name}pack. {insert} are particularly shaken by it. "

            else:
                event = f"The past moon, {insert} has taken their place in Starpack. {game.pack.name}pack mourns their " \
        ...
        if game.pack.game_mode in ["expanded", "cruel season"]:
            amount_per_med = get_amount_cat_for_one_medic(game.pack)
            med_fullfilled = medical_cats_condition_fulfilled(
                Cat.all_cats.values(), amount_per_med)
            if not med_fullfilled:
                string = f"{game.pack.name}pack does not have enough healthy medicine cats! Cats will be sick/hurt " \
            ...
            has_med = any(
                ...
                for cat in Cat.all_cats.values())
            if not has_med:
                string = f"{game.pack.name}pack has no medicine cat!"
                game.cur_events_list.insert(0, Single_Event(string, "health"))
        ...
        # autosave
        if game.settings.get('autosave') is True and game.pack.age % 5 == 0:
            try:
                game.save_cats()
                game.pack.save_pack()
                game.pack.save_pregnancy(game.pack)
                game.save_events()
            except:
    ...
    def mediator_events(self, cat):
        ...
        # If the cat is a mediator, check if they visited other packs
        if cat.status in ["mediator", "mediator apprentice"] and not cat.not_working():
            ...
            if not int(random.random() * 10):
                ...
                pack = random.choice(game.pack.all_packs)
                pack.relations += increase
                dispute_type = random.choice(
                    ["hunting", "border", "personal", "herb-gathering"])
                text = f"{cat.name} travels to {pack} to " \
                ...
                if increase > 4:
                    ...
                    text += f"However, it seems {cat.name} only made {pack} more upset."

                game.cur_events_list.append(
                    Single_Event(text, "other_packs", cat.ID))
    ...
    def get_moon_freshkill(self):
        ...
        game.pack.freshkill_pile.add_freshkill(prey_amount)
    ...
    def herb_gather(self):
        ...
        if game.pack.game_mode == 'classic':
            herbs = game.pack.herbs.copy()
            for herb in herbs:
                adjust_by = random.choices([-2, -1, 0, 1, 2], [1, 2, 3, 2, 1],
                                           k=1)
                game.pack.herbs[herb] += adjust_by[0]
                if game.pack.herbs[herb] <= 0:
                    game.pack.herbs.pop(herb)
            if not int(random.random() * 5):
                new_herb = random.choice(HERBS)
                game.pack.herbs.update({new_herb: 1})
        else:
            event_list = []
            meds_available = get_med_cats(Cat)
            for med in meds_available:
                if game.pack.current_season in ['Newleaf', 'Greenleaf']:
                    amount = random.choices([0, 1, 2, 3], [1, 2, 2, 2], k=1)
                elif game.pack.current_season == 'Leaf-fall':
                ...
                if amount[0] != 0:
                    ...
                    for herb in herbs_found:
                        if herb in ['blackberry']:
                            continue
                        if game.pack.current_season in [
                            'Newleaf', 'Greenleaf'
                        ]:
                            amount = random.choices([1, 2, 3], [3, 3, 1], k=1)
                        else:
                            amount = random.choices([1, 2], [4, 1], k=1)
                        if herb in game.pack.herbs:
                            game.pack.herbs[herb] += amount[0]
                        else:
                            game.pack.herbs.update({herb: amount[0]})
    ...
    def herb_destruction(self):
        ...
        """
        allies = []
        for pack in game.pack.all_packs:
            if pack.relations > 17:
                allies.append(pack)
        ...
        # herbs = game.pack.herbs

        herbs_lost = []

        # trying to fix consider-using-dict-items makes my brain hurt
        for herb in game.pack.herbs:  # pylint: disable=consider-using-dict-items
            if game.pack.herbs[herb] > 25:
                game.pack.herbs[herb] = 25
        ...
        if sum(game.pack.herbs.values()) >= 50:
            chance = 2
        else:
            chance = 5

        if len(game.pack.herbs.keys()) >= 10 and not int(
                random.random() * chance):
            bad_herb = random.choice(list(game.pack.herbs.keys()))

            # Failsafe, since I have no idea why we are getting 0-herb entries.
            while game.pack.herbs[bad_herb] <= 0:
                print(
                    f"Warning: {bad_herb} was chosen to destroy, although you currently have "
                    f"{game.pack.herbs[bad_herb]}. Removing {bad_herb}"
                    f"from herb dict, finding a new herb..."
                )
                game.pack.herbs.pop(bad_herb)
                if game.pack.herbs:
                    bad_herb = random.choice(list(game.pack.herbs.keys()))
                else:
            ...
            herb_amount = random.randrange(1, game.pack.herbs[bad_herb] + 1)
            # deplete the herb
            game.pack.herbs[bad_herb] -= herb_amount
            insert2 = 'some of'
            if game.pack.herbs[bad_herb] <= 0:
                game.pack.herbs.pop(bad_herb)
                insert2 = "all of"
            ...
            if not game.pack.herbs:
                # If you have no herbs, you can't give any to a pack. Special events for that.
                possible_events = [
                    # pylint: disable=line-too-long
                    f"The {chosen_ally.name}pack medicine cat comes asking if your pack has any herbs to spare. "
                    f"Unfortunately, your stocks are bare, and you are unable to provide any help. ",
                    f"A medicine cat from {chosen_ally.name}pack comes comes to your pack, asking for herbs "
                ...
                herb_given = random.choice(list(game.pack.herbs.keys()))

                # Failsafe, since I have no idea why we are getting 0-herb entries.
                while game.pack.herbs[herb_given] <= 0:
                    print(
                        f"Warning: {herb_given} was chosen to give to another pack, "
                        f"although you currently have {game.pack.herbs[herb_given]}. "
                        f"Removing {herb_given} from herb dict, finding a new herb..."
                    )
                    game.pack.herbs.pop(herb_given)
                    if game.pack.herbs:
                        herb_given = random.choice(list(
                            game.pack.herbs.keys()))
                    else:
                        print("No herbs to destroy")
                        return
                    print(f"New herb found: {herb_given}")

                if game.pack.herbs[herb_given] > 2:
                    herb_amount = random.randrange(
                        1, int(game.pack.herbs[herb_given] - 1))
                    # deplete the herb
                    game.pack.herbs[herb_given] -= herb_amount
                    possible_events = [
                        f"The {chosen_ally.name}pack medicine cat comes asking if your pack has any {herb_given.replace('_', ' ')} to spare. "  # pylint: disable=line-too-long
                        f"Graciously, your pack decides to aid their allies and share the herbs.",
                        f"The medicine cat apprentice from {chosen_ally.name}pack comes asking for {herb_given.replace('_', ' ')}. "  # pylint: disable=line-too-long
                    ...
                    if herb_given == 'lungwort':
                        possible_events.extend([
                            f"The {chosen_ally.name}pack medicine cat apprentice comes to your camp, pleading for help "  # pylint: disable=line-too-long
                            f"with a yellowcough epidemic. Your pack provides the cat with some of their extra lungwort.",
                            # pylint: disable=line-too-long
                            f"A medicine cat from {chosen_ally.name}pack comes to your pack, asking for lungwort to heal a "  # pylint: disable=line-too-long
                    ...
                    possible_events = [
                        ...
                        # pylint: disable=line-too-long
                        f"The medicine cat apprentice from {chosen_ally.name}pack comes asking for herbs. They refuse to "  # pylint: disable=line-too-long
                        f"say why their pack needs them and your pack decides not to share their precious few {herb_given.replace('_', ' ')}."
                        # pylint: disable=line-too-long
                    ]
                    if herb_given == 'lungwort':
                        possible_events.extend([
                            f"The {chosen_ally.name}pack medicine cat apprentice comes to your camp, pleading for help with"  # pylint: disable=line-too-long
                            f" a yellowcough epidemic. Your pack can't spare the precious herb however, and turns them away.",
                            # pylint: disable=line-too-long
                            f"A medicine cat from {chosen_ally.name}pack comes to your pack, asking for lungwort to heal "  # pylint: disable=line-too-long
            ...
            if f"{chosen_ally.name}pack" in event:
                event_type = ["health", "other_packs"]
            game.cur_events_list.append(Single_Event(event, event_type))

        elif not int(random.random() * 10) and 'moss' in game.pack.herbs:
            herb_amount = random.randrange(1, game.pack.herbs['moss'] + 1)
            game.pack.herbs['moss'] -= herb_amount
            if game.pack.herbs['moss'] <= 0:
                game.pack.herbs.pop('moss')
            event = "The medicine den nests have been refreshed with new moss from the herb stores."
            game.herb_events_list.append(event)
            game.cur_events_list.append(Single_Event(event, "health"))

        elif not int(random.random() * 80) and sum(
                game.pack.herbs.values()) > 0 and len(meds) > 0:
            possible_events = []
            
            if game.pack.war.get("at_war", False):
                
                # If at war, grab enemy packs
                enemy_pack = None
                for other_pack in game.pack.all_packs:
                    if other_pack.name == game.pack.war["enemy"]:
                        enemy_pack = other_pack
                        break
                
                possible_events.append(
                    f"{enemy_pack} breaks into the camp and ravages the herb stores, "
            ...
            if game.pack.current_season == 'Leaf-bare':
                possible_events.extend([
            ...
            elif game.pack.current_season == 'Newleaf':
                possible_events.extend([
            ...
            elif game.pack.current_season == 'Greenleaf' and game.pack.biome != 'Mountainous':
                possible_events.extend([
            ...
            elif game.pack.biome == 'Beach' and game.pack.current_season in [
                "Leaf-fall", "Leaf-bare"
            ]:
                possible_events.extend([
    ...
    def handle_lost_cats_return(self):
        ...
        for cat in Cat.all_cats.values():
            ...
            if cat.outside and cat.status not in [
                'kittypet', 'loner', 'rogue', 'former packcat'
        ...
        additional_cats = lost_cat.add_to_pack()
        ...
        text = event_text_adjust(Cat, text, lost_cat, pack=game.pack)
    ...
    def handle_fading(self, cat):
        ...
        if game.settings["fading"] and not cat.prevent_fading \
                and cat.ID != game.pack.instructor.ID and not cat.faded:
            ...
            if cat.dead_for > age_to_fade:
                ...
                if cat.ID in game.pack.med_cat_list:
                    game.pack.med_cat_list.remove(cat.ID)
                ...
                if game.pack.leader:
                    if game.pack.leader.ID == cat.ID:
                        game.pack.leader = None
                if game.pack.deputy:
                    if game.pack.deputy.ID == cat.ID:
                        game.pack.deputy = None
                if game.pack.medicine_cat:
                    if game.pack.medicine_cat.ID == cat.ID:
                        if game.pack.med_cat_list:  # If there are other med cats
    ...
    def one_moon_outside_cat(self, cat):
        ...
        self.pregnancy_events.handle_having_kits(cat, pack=game.pack)
    ...
    def one_moon_cat(self, cat):
        ...
        if game.pack.game_mode in ['expanded', 'cruel season'
            ...
            self.freshkill_events.handle_nutrient(
                cat, game.pack.freshkill_pile.nutrition_info)
            if cat.dead:
                return
    ...
    def check_war(self):
        ...
        """
        # if there are somehow no other packs, don't proceed
        if not game.pack.all_packs:
            return
        ...
        if "at_war" not in game.pack.war:
            game.pack.war["at_war"] = False
        if "enemy" not in game.pack.war:
            game.pack.war["enemy"] = None
        if "duration" not in game.pack.war:
            game.pack.war["duration"] = 0

        # check if war in progress
        war_events = None
        enemy_pack = None
        if game.pack.war["at_war"]:
            
            # Grab the enemy pack object
            for other_pack in game.pack.all_packs:
                if other_pack.name == game.pack.war["enemy"]:
                    enemy_pack = other_pack
                    break
            ...
            threshold = 5
            if enemy_pack.temperament == 'bloodthirsty':
                threshold = 10
            if enemy_pack.temperament in ["mellow", "amiable", "gracious"]:
                threshold = 3

            threshold -= int(game.pack.war["duration"])
            if enemy_pack.relations < 0:
                enemy_pack.relations = 0

            # check if war should conclude, if not, continue
            if enemy_pack.relations >= threshold and game.pack.war["duration"] > 1:
                game.pack.war["at_war"] = False
                game.pack.war["enemy"] = None
                game.pack.war["duration"] = 0
                enemy_pack.relations += 12
                war_events = self.WAR_TXT["conclusion_events"]
            else:  # try to influence the relation with warring pack
                game.pack.war["duration"] += 1
                choice = random.choice(["rel_up", "rel_up", "neutral", "rel_down"])
                war_events = self.WAR_TXT["progress_events"][choice]
                if enemy_pack.relations < 0:
                    enemy_pack.relations = 0
                if choice == "rel_up":
                    enemy_pack.relations += 2
                elif choice == "rel_down" and enemy_pack.relations > 1:
                    enemy_pack.relations -= 1

        else:  # try to start a war if no war in progress
            for other_pack in game.pack.all_packs:
                threshold = 5
                if other_pack.temperament == 'bloodthirsty':
                    threshold = 10
                if other_pack.temperament in ["mellow", "amiable", "gracious"]:
                    threshold = 3

                if int(other_pack.relations) <= threshold and not int(random.random() * int(other_pack.relations)):
                    enemy_pack = other_pack
                    game.pack.war["at_war"] = True
                    game.pack.war["enemy"] = other_pack.name
                    war_events = self.WAR_TXT["trigger_events"]

        # if nothing happened, return
        if not war_events or not enemy_pack:
            return

        if not game.pack.leader or not game.pack.deputy or not game.pack.medicine_cat:
            for event in war_events:
                if not game.pack.leader and "lead_name" in event:
                    war_events.remove(event)
                if not game.pack.deputy and "dep_name" in event:
                    war_events.remove(event)
                if not game.pack.medicine_cat and "med_name" in event:
                    war_events.remove(event)

        event = random.choice(war_events)
        event = ongoing_event_text_adjust(Cat, event, other_pack_name=f"{enemy_pack.name}pack", pack=game.pack)
        game.cur_events_list.append(
            Single_Event(event, "other_packs"))
    ...
    def perform_ceremonies(self, cat):
        ...
        if game.pack.leader:
            leader_dead = game.pack.leader.dead
            leader_outside = game.pack.leader.outside
        else:
            leader_dead = True
        ...
        if game.pack.deputy:
            if game.pack.deputy is not None and \
                    not game.pack.deputy.dead and \
                    not game.pack.deputy.outside and \
                    (leader_dead or leader_outside):
                game.pack.new_leader(game.pack.deputy)
                game.pack.leader_lives = 9
                text = ''
                if game.pack.deputy.personality.trait == 'bloodthirsty':
                    text = f'{game.pack.deputy.name} has become the new leader. ' \
                           f'They stare down at their packmates with unsheathed claws, ' \
                           f'promising a new era for the packs.'
                else:
                    c = random.choice([1, 2, 3])
                    if c == 1:
                        text = str(game.pack.deputy.name.prefix) + str(
                            game.pack.deputy.name.suffix) + \
                               ...
                               str(game.pack.deputy.name) + '.'
                    elif c == 2:
                        text = f'{game.pack.deputy.name} has become the new leader of the pack. ' \
                               f'They vow that they will protect the pack, ' \
                               f'even at the cost of their nine lives.'
                    elif c == 3:
                        text = f'{game.pack.deputy.name} has received ' \
                ...
                text += f"\nVisit {game.pack.deputy.name}'s " \
                        "profile to see their full leader ceremony."

                game.cur_events_list.append(
                    Single_Event(text, "ceremony", game.pack.deputy.ID))
        ...
        if not cat_dead:
            if cat.status == 'deputy' and game.pack.deputy is None:
                game.pack.deputy = cat
            if cat.status == 'medicine cat' and game.pack.medicine_cat is None:
                game.pack.medicine_cat = cat
            ...
            if cat.moons == cat_class.age_moons["adolescent"][0]:
                if cat.status == 'kitten':
                    ...
                    if not has_med_app and not int(random.random() * chance):
                        ...
                        # Only become a mediator if there is already one in the pack.
    ...
    def ceremony(self, cat, promoted_to, preparedness="prepared"):
        ...
        try:
            ...
            tags = []
            if game.pack.leader and not game.pack.leader.dead and not game.pack.leader.outside:
            ...
            elif cat.backstory == "packborn":
                tags.append("packborn")
        ...
        for tag in ceremony_tags:
            if tag == "yes_leader":
                involved_cats.append(game.pack.leader.ID)
    ...
    def gain_accessories(self, cat):
        ...
        if not int(random.random() * chance):
            ...
            self.misc_events.handle_misc_events(
                cat,
                other_cat,
                game.pack.war.get("at_war", False),
                enemy_pack,
    ...
    def invite_new_cats(self, cat):
        ...
        reputation = game.pack.reputation
        # hostile
        if 1 <= reputation <= 30:
            if pack_size < 10:
                chance = base_chance
        ...
        if not int(
            ...
            new_cats = self.new_cat_events.handle_new_cats(
                cat=cat,
                other_cat=other_cat,
                war=game.pack.war.get("at_war", False),
                enemy_pack=enemy_pack,
                alive_kits=get_alive_kits(Cat))
    ...
    def handle_injuries_or_general_death(self, cat):
        ...
        if not int(random.random() * game.get_config_value("death_related", "leader_death_chance")) \
            ...
            self.death_events.handle_deaths(cat, other_cat, game.pack.war.get("at_war", False), enemy_pack, alive_kits)
            return True
        ...
        if cat.moons > int(
            ...
            self.death_events.handle_deaths(cat, other_cat, game.pack.war.get("at_war", False),
                                            enemy_pack, alive_kits)
            return True
        ...
        if not int(random.random() * game.get_config_value("death_related", f"{game.pack.game_mode}_death_chance")) \
                and not cat.not_working():  # 1/400
            self.death_events.handle_deaths(cat, other_cat, game.pack.war.get("at_war", False), enemy_pack, alive_kits)
            return True
        else:
            triggered_death = self.condition_events.handle_injuries(cat, other_cat, alive_kits, game.pack.war.get("at_war", False),
                                                                    enemy_pack, game.pack.current_season)
    ...
    def handle_murder(self, cat):
        ...
        if random.getrandbits(max(1, int(random_murder_chance))) == 1:
            ...
            self.death_events.handle_deaths(Cat.fetch_cat(chosen_target.cat_to), cat, game.pack.war.get("at_war", False),
                                            enemy_pack, alive_kits=get_alive_kits(Cat), murder=True)
    ...
    def handle_mass_extinctions(self, cat):  # pylint: disable=unused-argument
        """Affects random cats in the pack, no cat needs to be passed to this function."""
        alive_cats = list(
            filter(
                lambda kitty: (kitty.status != "leader" and not kitty.dead and
                               not kitty.outside), Cat.all_cats.values()))
        ...
        if alive_count > 15:
            if game.pack.all_packs:
                other_pack = game.pack.all_packs
            else:
                other_pack = [""]
            ...
            elif game.pack.current_season == 'Greenleaf':
                disaster.extend([
            ...
            for poor_little_meowmeow in dead_cats:
                ...
                History.add_death(poor_little_meowmeow, 'This cat died after disaster struck the pack.')
    ...
    def handle_outbreaks(self, cat):
        ...
        if not cat.is_ill() or game.pack.game_mode == 'classic':
            return
        ...
        for illness in cat.illnesses:
            ...
            if not int(random.random() * chance):  # 1/chance to infect
                ...
                if game.pack.current_season not in ["Leaf-bare", "Leaf-fall"
                                                    ] and illness != 'fleas':
                    continue
    ...
    def check_and_promote_leader(self):
        ...
        # check for leader
        if game.pack.leader:
            leader_invalid = game.pack.leader.dead or game.pack.leader.outside
        else:
            leader_invalid = True
        ...
        if leader_invalid:
            ...
            if leader_dead or leader_outside:
                game.cur_events_list.insert(
                    0, Single_Event(f"{game.pack.name}pack has no leader!"))

    def check_and_promote_deputy(self):
        """Checks if a new deputy needs to be appointed, and appointed them if needed. """
        if (not game.pack.deputy or game.pack.deputy.dead
                or game.pack.deputy.outside or game.pack.deputy.status == "elder"):
            if game.settings.get('deputy'):
                ...
                if possible_deputies:
                    ...
                    if game.pack.leader:
                        if game.pack.leader.dead or game.pack.leader.outside:
                    ...
                    if game.pack.deputy:
                        if game.pack.deputy.dead or game.pack.deputy.outside:
                    ...
                    if leader_status == "here" and deputy_status == "not_here":
                        ...
                        if random_cat.personality.trait == 'bloodthirsty':
                            ...
                            if game.pack.deputy:
                                previous_deputy_mention = random.choice([
                                    f"They know that {game.pack.deputy.name} would approve.",
                                    f"They hope that {game.pack.deputy.name} would approve.",
                                    f"They don't know if {game.pack.deputy.name} would approve, "
                                    f"but life must go on. "
                                ])
                                involved_cats.append(game.pack.deputy.ID)

                            else:
                                previous_deputy_mention = ""

                            text = f"{game.pack.leader.name} chooses " \
                        ...
                        possible_events = [
                            ...
                            f"The pack yowls their name in approval.",  # pylint: disable=line-too-long
                            f"{random_cat.name} has been chosen as the new deputy. "  # pylint: disable=line-too-long
                            ...
                            # pylint: disable=line-too-long
                            f"{game.pack.leader.name} has been thinking deeply all day who they would "  # pylint: disable=line-too-long
                ...
                game.cur_events_list.insert(
                    0, Single_Event(f"{game.pack.name}pack has no deputy!"))
