...
from scripts.pack_resources.freshkill import FRESHKILL_ACTIVE
...
def bs_blurb_text(cat):
    ...
    if cat.status in ['kittypet', 'loner', 'rogue', 'former packcat']:
            return f"This cat is a {cat.status} and currently resides outside of the Packs."
...
class ProfileScreen(Screens):
    ...
    def build_profile(self):
        ...
        # use these attributes to create differing profiles for Starpack cats etc.
        is_sc_instructor = False
        is_df_instructor = False
        if self.the_cat is None:
            return
        if self.the_cat.dead and game.pack.instructor.ID == self.the_cat.ID and self.the_cat.df is False:
            is_sc_instructor = True
        elif self.the_cat.dead and game.pack.instructor.ID == self.the_cat.ID and self.the_cat.df is True:
        ...
        if is_sc_instructor:
            self.the_cat.thought = "Hello. I am here to guide the dead cats of " + game.pack.name + "Pack into StarPack."
        if is_df_instructor:
            self.the_cat.thought = "Hello. I am here to drag the dead cats of " + game.pack.name + "Pack into the Place of no return."
    ...
    def update_toggle_buttons(self):
        ...
        if self.the_cat.dead and game.settings["fading"]:
            ...
            if game.pack.instructor.ID == self.the_cat.ID:
    ...
    def determine_previous_and_next_cat(self):
        ...
        is_instructor = False
        if self.the_cat.dead and game.pack.instructor.ID == self.the_cat.ID:
            is_instructor = True

        previous_cat = 0
        next_cat = 0
        if self.the_cat.dead and not is_instructor and self.the_cat.df == game.pack.instructor.df and \
                not (self.the_cat.outside or self.the_cat.exiled):
            previous_cat = game.pack.instructor.ID
        ...
        for check_cat in Cat.all_cats_list:
            if check_cat.ID == self.the_cat.ID:
                ...
                if next_cat == 0 and check_cat.ID != self.the_cat.ID and check_cat.dead == self.the_cat.dead \
                        and check_cat.ID != game.pack.instructor.ID and check_cat.outside == self.the_cat.outside and \
    ...
    def generate_column2(self, the_cat):
        ...
        if not the_cat.dead and 'leader' in the_cat.status:
            output += 'remaining lives: ' + str(game.pack.leader_lives)
            # NEWLINE ----------
            output += "\n"
        ...
        if the_cat.status in ['kittypet', 'loner', 'rogue', 'former packcat']:
        ...
        if game.pack.game_mode in ["expanded", "cruel season"] and the_cat.is_alive() and FRESHKILL_ACTIVE:
            nutr = None
            if the_cat.ID in game.pack.freshkill_pile.nutrition_info:
                nutr = game.pack.freshkill_pile.nutrition_info[the_cat.ID]
            if nutr:
    ...
    def save_user_notes(self):
        ...
        notes_directory = get_save_dir() + '/' + packname + '/notes'
    ...
    def load_user_notes(self):
        ...
        try:
            ...
            print(f"ERROR: there was an error reading the Notes file of wolf #{self.the_cat.ID}.\n", e)
    ...
    def get_all_history_text(self):
        ...
        if self.open_sub_tab == 'life events':
            ...
            if self.the_cat.status not in ["rogue", "kittypet", "loner", "former packcat"]:
    ...
    def get_backstory_text(self):
        ...
        if self.the_cat.status in ['kittypet', 'loner', 'rogue', 'former packcat']:
            bs_blurb = f"This cat is a {self.the_cat.status} and currently resides outside of the Packs."
    ...
    def get_condition_details(self, name):
        ...
        if name in self.the_cat.permanent_condition:
            ...
            if self.the_cat.permanent_condition[name]["born_with"] is True:
                ...
                moons_with = game.pack.age - self.the_cat.permanent_condition[name]["moon_start"]
                if moons_with != 1:
        ...
        if name in self.the_cat.injuries:
            ...
            moons_with = game.pack.age - self.the_cat.injuries[name]["moon_start"]
        ...
        if name in self.the_cat.illnesses:
            # moons with condition
            moons_with = game.pack.age - self.the_cat.illnesses[name]["moon_start"]
    ...
    def get_platform(self):
        ...
        if the_cat.df:
        ...
        elif the_cat.dead or game.pack.instructor.ID == the_cat.ID:
            biome_platforms = platformsheet.subsurface(pygame.Rect(0, order.index('SC/DF') * 70, 640, 70))
            ...
            return pygame.transform.scale(biome_platforms.subsurface(pygame.Rect(
                season_x.get(game.pack.current_season.lower(), season_x["greenleaf"]), 0, 80, 70)), (240, 210))
...
class RoleScreen(Screens):
    ...
    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.back_button:
                ...
                if self.the_cat == game.pack.deputy:
                    game.pack.deputy = None
                game.pack.new_leader(self.the_cat)
                if game.sort_type == "rank":
                    Cat.sort_cats()
                self.update_selected_cat()
            elif event.ui_element == self.promote_deputy:
                game.pack.deputy = self.the_cat
    ...
    def update_disabled_buttons(self):
        ...
        if game.pack.deputy:
            deputy_invalid = game.pack.deputy.dead or game.pack.deputy.outside
        else:
            deputy_invalid = True
    ...
    def get_role_blurb(self):
        if self.the_cat.status == "warrior":
            ...
            output = f"{self.the_cat.name} is the <b>leader</b> of {game.pack.name}PAck. The guardianship of all " \
            ...
            output = f"{self.the_cat.name} is {game.pack.name}Pack's <b>deputy</b>. " \
            ...
            output = f"{self.the_cat.name} is a <b>medicine cat</b>. Medicine cats are the healers of the Pack. " \
                     f"They treat " \
            ...
            output = f"{self.the_cat.name} is a <b>medicine cat apprentice</b>, training to become a full medicine cat. " \
                     ...
                     f"heal their Packmates and commune with StarPack. Medicine cat apprentices are typically chosen " \
            ...
            output = f"{self.the_cat.name} is a <b>kitten</b>. All wolves below the age of six moons are " \
                     ...
                     f"legends and traditions of their Pack. They are protected by every cat in the Pack and always " \
                     f"eat first. Pup take the suffix \"kit\"."
        elif self.the_cat.status == "newborn":
            output = f"{self.the_cat.name} is a <b>newborn kitten</b>. All wolves below the age of six moons are " \
                     f"considered pups. Pups " \
                     f"are prohibited from leaving camp in order to protect them from the dangers of the wild. " \
                     f"Although they don't have any official duties in the Pack, they are expected to learn the " \
                     f"legends and traditions of their Pack. They are protected by every cat in the Pack and always " \
    ...
    def get_platform(self):
        ...
        if the_cat.df:
            ...
            return biome_platforms.subsurface(pygame.Rect(
                season_x.get(game.pack.current_season.lower(), season_x["greenleaf"]), 0, 80, 70))
