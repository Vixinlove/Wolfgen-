...
class PackScreen(Screens):
    ...
    def handle_event(self, event):
        if game.switches['window_open']:
            ...
            if event.key == pygame.K_RIGHT:
                self.change_screen('starpack screen')
    ...
    def screen_switches(self):
        ...
        if game.pack.biome + game.pack.camp_bg in game.pack.layouts:
            self.layout = game.pack.layouts[game.pack.biome + game.pack.camp_bg]
        else:
            self.layout = game.pack.layouts["default"]

        self.choose_cat_positions()
        
        self.set_disabled_menu_buttons(["camp_screen"])
        self.update_heading_text(f'{game.pack.name}pack')
        ...
        self.save_button_saved_state = pygame_gui.elements.UIImage(
            ...
            pygame.transform.scale(
                image_cache.load_image('resources/images/save_pack_saved.png'),
                (228, 60)))
        self.save_button_saved_state.hide()
        self.save_button_saving_state = pygame_gui.elements.UIImage(
            scale(pygame.Rect((686, 1286), (228, 60))),
            pygame.transform.scale(
                image_cache.load_image('resources/images/save_pack_saving.png'),
                (228, 60)))
    ...
    def update_camp_bg(self):
        ...
        camp_nr = game.pack.camp_bg

        if camp_nr is None:
            camp_nr = 'camp1'
            game.pack.camp_bg = camp_nr

        available_biome = ['Forest', 'Mountainous', 'Plains', 'Beach']
        biome = game.pack.biome
        if biome not in available_biome:
            biome = available_biome[0]
            game.pack.biome = biome
        biome = biome.lower()
    ...
    def choose_cat_positions(self):
        """Determines the positions of cat on the pack screen."""
        ...
        for x in game.pack.pack_cats:
            ...
            if Cat.all_cats[x].status in ['apprentice', 'mediator apprentice']:
                ...
                game.pack.leader.placement = self.choose_nonoverlapping_positions(first_choices, all_dens,
                                                                                  [1, 200, 1, 1, 1, 1, 1])
                                                                                  

    def update_buttons_and_text(self):
        if game.switches['saved_pack']:
            self.save_button_saving_state.hide()
...
class StarpackScreen(Screens):
    ...
    def __init__(self, name=None):
        ...
        self.starpack_bg = pygame.transform.scale(
            pygame.image.load("resources/images/starpackbg.png").convert(),
            (screen_x, screen_y))
        self.search_bar_image = pygame.transform.scale(
            pygame.image.load("resources/images/search_bar.png").convert_alpha(), (456, 68))
        self.pack_name_bg = pygame.transform.scale(
            image_cache.load_image("resources/images/pack_name_bg.png").convert_alpha(), (380, 70))
    ...
    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            ...
            if event.key == pygame.K_LEFT:
                self.change_screen("pack screen")
            elif event.key == pygame.K_RIGHT:
                self.change_screen('patrol screen')

    def exit_screen(self):
        self.hide_menu_buttons()
        self.starpack_button.kill()
    ...
    def get_dead_cats(self):
        self.dead_cats = [game.pack.instructor] if not game.pack.instructor.df else []
        for the_cat in Cat.all_cats_list:
            if the_cat.dead and the_cat.ID != game.pack.instructor.ID and not the_cat.outside and not the_cat.df and\
                    not the_cat.faded:
    ...
    def screen_switches(self):
        ...
        self.starpack_button = UIImageButton(scale(pygame.Rect((230, 270), (68, 68))), "", object_id="#starpack_button"
                                             , manager=MANAGER)
        self.starpack_button.disable()
    ...
    def update_page(self):
        ...
        if self.current_listed_cats:
            for cat in self.chunks(self.current_listed_cats, 20)[self.list_page - 1]:
                ...
                if game.pack.pack_settings["show_fav"] and cat.favourite:
...
class DFScreen(Screens):
    ...
    def __init__(self, name=None):
        ...
        self.pack_name_bg = pygame.transform.scale(
            image_cache.load_image("resources/images/pack_name_bg.png").convert_alpha(), (int(380 / 1600 * screen_x),
                                                                                          int(68 / 1400 * screen_y)))

    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.starpack_button:
                self.change_screen('starpack screen')
            elif event.ui_element == self.unknown_residence_button:
                self.change_screen('unknown residence screen')
            elif event.ui_element == self.to_living_button:
    ...
    def get_dead_cats(self):
        self.dead_cats = [game.pack.instructor] if game.pack.instructor.df else []

        for the_cat in Cat.all_cats_list:
            if the_cat.dead and the_cat.ID != game.pack.instructor.ID and the_cat.df and \
                    not the_cat.faded:
                self.dead_cats.append(the_cat)
...
class ListScreen(Screens):
    ...
    def __init__(self, name=None):
        ...
        self.previous_page_button = None
        self.next_page_button = None
        self.outside_pack_button = None
        self.your_pack_button = None
    ...
    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.outside_pack_button:
    ...
    def screen_switches(self):
        ...
        self.your_pack_button = UIImageButton(scale(pygame.Rect((230, 270), (68, 68))), "",
                                              object_id="#your_pack_button"
                                              , manager=MANAGER)
        self.your_pack_button.disable()
        self.outside_pack_button = UIImageButton(scale(pygame.Rect((298, 270), (68, 68))), "",
                                                 object_id="#outside_pack_button", manager=MANAGER)
        ...
        self.pack_button = UIImageButton(scale(pygame.Rect((230, 200), (68, 68))), "",
                                                 object_id="#pack_button", manager=MANAGER)
        
        if game.pack.pack_settings["show_fav"]:
            self.filter_not_fav.hide()
        else:
            self.filter_fav.hide()
    ...
    def exit_screen(self):
        self.hide_menu_buttons()
        self.your_pack_button.kill()
        self.outside_pack_button.kill()
...
class AllegiancesScreen(Screens):
    ...
    def screen_switches(self):
        # Heading
        self.heading = pygame_gui.elements.UITextBox(f'{game.pack.name}Pack Allegiances',
    ...
    def get_allegiances_text(self):
        ...
        outputs = []
        if game.pack.leader and not (game.pack.leader.dead or game.pack.leader.outside):
                outputs.append([
                    '<b><u>LEADER</u></b>',
                    self.generate_one_entry(game.pack.leader)
                ])

        # Deputy Box:
        if game.pack.deputy and not (game.pack.deputy.dead or game.pack.deputy.outside):
            outputs.append([
                '<b><u>DEPUTY</u></b>',
                self.generate_one_entry(game.pack.deputy)
            ])
...
class MedDenScreen(Screens):
    ...
    def screen_switches(self):
        ...
        if self.meds:
            ...
            herb_amount = sum(game.pack.herbs.values())
            ...
            if herb_amount == 0:
                ...
                if len(self.meds) == 1:
                    med_concern = f"StarPack has blessed them with plentiful herbs and the medicine cat sends their thanks to Silverpelt."
                else:
                    med_concern = f"StarPack has blessed them with plentiful herbs and the medicine cats send their thanks to Silverpelt."
    ...
    def draw_med_den(self):
        sorted_dict = dict(sorted(game.pack.herbs.items()))
