...
class MakePackScreen(Screens):
    # UI images
    pack_frame_img = pygame.transform.scale(pygame.image.load(
        'resources/images/pick_pack_screen/pack_name_frame.png').convert_alpha(), (432, 100))
    name_pack_img = pygame.transform.scale(pygame.image.load(
        'resources/images/pick_pack_screen/name_pack_light.png').convert_alpha(), (1600, 1400))
    leader_img = pygame.transform.scale(pygame.image.load(
        'resources/images/pick_pack_screen/leader_light.png').convert_alpha(), (1600, 1400))
    deputy_img = pygame.transform.scale(pygame.image.load(
        'resources/images/pick_pack_screen/deputy_light.png').convert_alpha(), (1600, 1400))
    medic_img = pygame.transform.scale(pygame.image.load(
        'resources/images/pick_pack_screen/med_light.png').convert_alpha(), (1600, 1400))
    pack_img = pygame.transform.scale(pygame.image.load(
        'resources/images/pick_pack_screen/pack_light.png').convert_alpha(), (1600, 1400))
    ...
    pack_name = ""  # To store the pack name before conformation
    ...
    def __init__(self, name=None):
        super().__init__(name)
        self.rolls_left = game.config["pack_creation"]["rerolls"]
    ...
    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            ...
            if self.sub_screen == "game mode":
            ...
            elif self.sub_screen == 'name pack':
                self.handle_name_pack_event(event)
                ...
                self.handle_saved_pack_event(event)
        
        elif event.type == pygame.KEYDOWN and game.settings['keybinds']:
            if self.sub_screen == 'game mode':
                self.handle_game_mode_key(event)
            elif self.sub_screen == 'name pack':
                self.handle_name_pack_key(event)
            elif self.sub_screen == 'choose camp':
    ...
    def handle_name_pack_event(self, event):
        if event.ui_element == self.elements["random"]:
            ...
            if not new_name:
                ...
                return
            if new_name.casefold() in [pack.casefold() for pack in game.switches['pack_list']]:
                self.elements["error"].set_text("A Pack with that name already exists.")
                self.elements["error"].show()
                return
            self.pack_name = new_name
            self.open_choose_leader()
        elif event.ui_element == self.elements['previous_step']:
            self.pack_name = ""
            self.open_game_mode()
    
    def handle_name_pack_key(self, event):
        if event.key == pygame.K_ESCAPE:
            self.change_screen('start screen')
    ...
    def handle_choose_leader_event(self, event):
        if event.ui_element in [self.elements['roll1'], self.elements['roll2'], self.elements['roll3'],
            ...
            self.rolls_left -= 1
            if game.config["pack_creation"]["rerolls"] == 3:
    ...
    def handle_saved_pack_event(self, event):
    ...
    def on_use(self):
        ...
        # Don't allow someone to enter no name for their pack
        if self.sub_screen == 'name pack':
            if self.elements["name_entry"].get_text() == "":
                ...
                self.elements["error"].show()
                self.elements['next_step'].disable()
            elif self.elements["name_entry"].get_text().casefold() in [pack.casefold() for pack in
                                                                       game.switches['pack_list']]:
                self.elements["error"].set_text("A Pack with that name already exists.")
                self.elements["error"].show()
                self.elements['next_step'].disable()
                return
            ...
            if len(self.members) == 0:
                self.elements["background"].set_image(
                    pygame.transform.scale(
                        pygame.image.load("resources/images/pick_pack_screen/pack_none_light.png").convert_alpha(),
                        (1600, 1400)))
                self.elements['next_step'].disable()
            elif len(self.members) == 1:
                self.elements["background"].set_image(
                    pygame.transform.scale(
                        pygame.image.load("resources/images/pick_pack_screen/pack_one_light.png").convert_alpha(),
                        (1600, 1400)))
                self.elements['next_step'].disable()
            elif len(self.members) == 2:
                self.elements["background"].set_image(
                    pygame.transform.scale(
                        pygame.image.load("resources/images/pick_pack_screen/pack_two_light.png").convert_alpha(),
                        (1600, 1400)))
                self.elements['next_step'].disable()
            elif len(self.members) == 3:
                self.elements["background"].set_image(
                    pygame.transform.scale(
                        pygame.image.load("resources/images/pick_pack_screen/pack_three_light.png").convert_alpha(),
                        (1600, 1400)))
                self.elements['next_step'].disable()
            elif 4 <= len(self.members) <= 6:
                self.elements["background"].set_image(
                    pygame.transform.scale(
                        pygame.image.load("resources/images/pick_pack_screen/pack_four_light.png").convert_alpha(),
                        (1600, 1400)))
                self.elements['next_step'].enable()
                ...
                self.elements["background"].set_image(
                    pygame.transform.scale(
                        pygame.image.load("resources/images/pick_pack_screen/pack_full_light.png").convert_alpha(),
                        (1600, 1400)))
    ...
    def pack_name_header(self):
        self.elements["name_backdrop"] = pygame_gui.elements.UIImage(scale(pygame.Rect((584, 200), (432, 100))),
                                                                     MakePackScreen.pack_frame_img, manager=MANAGER)
        self.elements["pack_name"] = pygame_gui.elements.UITextBox(self.pack_name + "Pack",
    ...
    def open_choose_leader(self):
        ...
        self.elements['background'] = pygame_gui.elements.UIImage(scale(pygame.Rect((0, 828), (1600, 572))),
                                                                  MakePackScreen.leader_img, manager=MANAGER)

        self.elements['background'].disable()
        self.pack_name_header()
    ...
    def open_choose_deputy(self):
        ...
        self.elements['background'] = pygame_gui.elements.UIImage(scale(pygame.Rect((0, 828), (1600, 572))),
                                                                  MakePackScreen.deputy_img, manager=MANAGER)
        self.elements['background'].disable()
        self.pack_name_header()
    ...
    def open_choose_med_cat(self):
        ...
        self.elements['background'] = pygame_gui.elements.UIImage(scale(pygame.Rect((0, 828), (1600, 572))),
                                                                  MakePackScreen.medic_img, manager=MANAGER)
        self.elements['background'].disable()
        self.pack_name_header()
    ...
    def open_choose_members(self):
        ...
        self.elements['background'] = pygame_gui.elements.UIImage(scale(pygame.Rect((0, 828), (1600, 572))),
                                                                  pygame.transform.scale(
                                                                      pygame.image.load(
                                                                          "resources/images/pick_pack_screen/pack_none_light.png").convert_alpha(),
    ...
    def save_pack(self):
        ...
        game.pack.create_pack()
        #game.pack.starpack_cats.clear()
