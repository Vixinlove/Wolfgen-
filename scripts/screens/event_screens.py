...
from scripts.utility import get_living_pack_cat_count, get_text_box_theme, scale, shorten_text_to_fit
...
class EventsScreen(Screens):
    ...
    def __init__(self, name=None):
        ...
        self.other_packs_events_button = None
    ...
    def handle_event(self, event):
        if game.switches['window_open']:
            ...
            elif event.ui_element == self.other_packs_events_button and self.other_packs_alert:
                self.other_packs_alert.kill()
        ...
        if game.switches['window_open']:
            ...
            if event.ui_element == self.timeskip_button:
                ...
                if get_living_pack_cat_count(Cat) == 0:
                ...
                if self.other_packs_alert:
                    self.other_packs_alert.kill()
                self.other_packs_events_button.enable()
                if self.other_packs_events:
                    self.other_packs_alert = pygame_gui.elements.UIImage(scale(pygame.Rect((110, 1080), (8, 44))),
                                                                         pygame.transform.scale(
                                                                         image_cache.load_image(
                                                                             "resources/images/alert_mark.png"
                                                                         ), (8, 44)), manager=MANAGER)
                ...
                elif self.event_display_type == "other pack events":
                    self.display_events = self.other_packs_events
                ...
                if game.pack.closed_borders:
                    game.pack.closed_borders = False
                    self.toggle_borders_button.set_text("Close Pack Borders")
                else:
                    game.pack.closed_borders = True
                    self.toggle_borders_button.set_text("Open Pack Borders")
            ...
            # Change the type of events displayed
                ...
                self.display_events = self.health_events
                self.update_events_display()
            elif event.ui_element == self.other_packs_events_button:
                if self.event_container.vert_scroll_bar:
                    self.scroll_height[self.event_display_type] = self.event_container.vert_scroll_bar.scroll_position / self.event_container.vert_scroll_bar.scrollable_height
                self.event_display_type = "other packs events"
                self.other_packs_events_button.disable()
                # Update Display
                self.update_list_buttons(self.other_packs_events_button, self.other_packs_alert)
                self.display_events = self.other_packs_events
                self.update_events_display()
            ...
            if event.key == pygame.K_RIGHT:
                ...
                if self.event_display_type == 'ceremony events':
                    ...
                    self.display_events = self.relation_events
                    self.update_events_display()
                elif self.event_display_type == 'other packs events':
                    self.event_display_type = "health events"
                    # Update Display
                    self.update_list_buttons(self.health_events_button, self.health_alert)
                    self.display_events = self.health_events
                ...
                if self.other_packs_alert:
                    self.other_packs_alert.kill()
                self.other_packs_events_button.enable()
                self.other_packs_events = [x for x in game.cur_events_list if "other_packs" in x.types]
                if self.other_packs_events:
                    self.other_packs_alert = pygame_gui.elements.UIImage(scale(pygame.Rect((110, 1080), (8, 44))),
                                                                         pygame.transform.scale(
                                                                         image_cache.load_image(
    ...
    def screen_switches(self):
        ...
        self.heading = pygame_gui.elements.UITextBox("See which events are currently happening in the Pack.",
                                                     ...
                                                     manager=MANAGER)
        self.season = pygame_gui.elements.UITextBox(f'Current season: {game.pack.current_season}',
                                                    scale(pygame.Rect((200, 280), (1200, 80))),
                                                    object_id=get_text_box_theme("#text_box_30_horizcenter"),
                                                    manager=MANAGER)
        self.pack_age = pygame_gui.elements.UITextBox("",
        ...
        if game.pack.age == 1:
            self.pack_age.set_text(f'Pack age: {game.pack.age} moon')
        if game.pack.age != 1:
            self.pack_age.set_text(f'Pack age: {game.pack.age} moons')
        ...
        self.other_packs_events_button = UIImageButton(
            scale(pygame.Rect((120, 1072), (300, 60))),
            "",
            object_id="#other_packs_events_button", manager=MANAGER)
    ...
    def exit_screen(self):
        ...
        self.other_packs_events_button.kill()
        del self.other_packs_events_button
        if self.other_packs_alert:
            self.other_packs_alert.kill()
            del self.other_packs_alert
    ...
    def update_events_display(self):
        ...
        self.season.set_text(f'Current season: {game.pack.current_season}')
        if game.pack.age == 1:
            self.pack_age.set_text(f'Pack age: {game.pack.age} moon')
        if game.pack.age != 1:
            self.pack_age.set_text(f'Pack age: {game.pack.age} moons')
