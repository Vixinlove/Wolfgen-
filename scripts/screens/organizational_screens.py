...
class StartScreen(Screens):
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            screens = {
                ...
                self.switch_pack_button: 'switch pack screen',
                self.new_pack_button: 'make pack screen',
            ...
            if event.ui_element in screens:
                ...
                if platform.system() == 'Darwin':
                    subprocess.Popen(["open", "-u", "https://discord.gg/packgen"])
                elif platform.system() == 'Windows':
                    os.system(f"start \"\" {'https://discord.gg/packgen'}")
                elif platform.system() == 'Linux':
                    subprocess.Popen(['xdg-open', "https://discord.gg/packgen"])
            elif event.ui_element == self.social_buttons['tumblr_button']:
                if platform.system() == 'Darwin':
                    subprocess.Popen(["open", "-u", "https://officialpackgen.tumblr.com/"])
                elif platform.system() == 'Windows':
                    os.system(f"start \"\" {'https://officialpackgen.tumblr.com/'}")
                elif platform.system() == 'Linux':
                    subprocess.Popen(['xdg-open', "https://officialpackgen.tumblr.com/"])
            elif event.ui_element == self.social_buttons['twitter_button']:
                if platform.system() == 'Darwin':
                    subprocess.Popen(["open", "-u", "https://twitter.com/Officialpackgen"])
                elif platform.system() == 'Windows':
                    os.system(f"start \"\" {'https://twitter.com/Officialpackgen'}")
                elif platform.system() == 'Linux':
                    subprocess.Popen(['xdg-open', "https://twitter.com/Officialpackgen"])
    ...
    def exit_screen(self):
        ...
        self.switch_pack_button.kill()
        self.new_pack_button.kill()
    ...
    def screen_switches(self):
        ...
        self.continue_button = UIImageButton(scale(
            ...
            manager=MANAGER)
        self.switch_pack_button = UIImageButton(
            scale(pygame.Rect((140, 710), (384, 70))),
            "",
            object_id="#switch_pack_button",
            manager=MANAGER)
        self.new_pack_button = UIImageButton(scale(
            pygame.Rect((140, 800), (384, 70))),
            "",
            object_id="#new_pack_button",
            manager=MANAGER)
        ...
        self.error_gethelp = pygame_gui.elements.UITextBox(
            ...
            '<br><a href="https://discord.gg/packgen">Discord</a>',  # pylint: disable=line-too-long
        ...
        try:
            ...
            if not get_version_info().is_source_build and not get_version_info().is_itch and get_version_info().upstream.lower() == "Thlumyn/packgen".lower() and \
        ...
        if game.pack is not None and game.switches['error_message'] == '':
            self.continue_button.enable()
        else:
            self.continue_button.disable()
        
        if len(game.switches['pack_list']) > 1:
            self.switch_pack_button.enable()
        else:
            self.switch_pack_button.disable()
        ...
        if game.pack is not None:
            ...
            for x in key_copy:
                if x not in game.pack.pack_cats:
                    game.pack.remove_cat(x)

        # LOAD settings
        game.load_settings()


class SwitchpackScreen(Screens):
    """
    TODO: DOCS
    """
            ...
            if game.switches['window_open']:
                ...
                for page in self.delete_buttons:
                    if event.ui_element in page:
                        DeleteCheck(
                            self.change_screen,
                            self.pack_name[self.page][page.index(
                                event.ui_element)])

                        return

                for page in self.pack_buttons:
                    if event.ui_element in page:
                        Pack.switch_packs(
                            self.pack_name[self.page][page.index(
                                event.ui_element)])
    ...
    def exit_screen(self):
        ...
        self.info.kill()
        del self.info
        self.current_pack.kill()
        del self.current_pack
    ...
    def screen_switches(self):
        ...
        self.screen = pygame.transform.scale(
            pygame.image.load(
                "resources/images/pack_saves_frame.png").convert_alpha(),
        ...
        self.current_pack = pygame_gui.elements.UITextBox(
            "",
            scale(pygame.Rect((200, 200), (1200, 140))),
            object_id=get_text_box_theme("#text_box_30_horizcenter"),
            manager=MANAGER)
        if game.pack:
            self.current_pack.set_text(
                f"The currently loaded pack is {game.pack.name}pack")
        else:
            self.current_pack.set_text("There is no pack currently loaded.")

        self.pack_list = game.read_packs()
        ...
        i = 0
        y_pos = 378
        for pack in self.pack_list[1:]:
            self.pack_name[-1].append(pack)
            self.pack_buttons[-1].append(
                pygame_gui.elements.UIButton(scale(
                    pygame.Rect((600, y_pos), (400, 78))),
                    pack + "pack",
                    object_id="#saved_pack",
                    manager=MANAGER))
            ...
            i += 1
            if i >= 8:
                self.pack_buttons.append([])
                self.pack_name.append([])
    ...
    def update_page(self):
        ...
        if self.page >= len(self.pack_buttons) - 1:
            self.next_page_button.disable()
        else:
            self.next_page_button.enable()

        self.page_number.set_text(
            f"Page {self.page + 1} of {len(self.pack_buttons)}")
        ...
        for button in self.pack_buttons[self.page]:
            button.show()
...
class StatsScreen(Screens):
    ...
    def screen_switches(self):
        ...
        for cat in Cat.all_cats.values():
            if not cat.dead and not (cat.outside or cat.exiled):
                ...
                if cat.status == 'warrior':
                    ...
                    medcat_num += 1
            elif (cat.status in ['kittypet', 'loner', 'rogue', 'former packcat']
        ...
        stats_text = f"Number of Living Cats: {living_num}\n\n" + \
                     ...
                     f"Number of Cats Outside the packs: {other_num}\n\n" + \
                     f"Number of Dead Cats: {starpack_num}"
