...
from scripts.pack import HERBS, Pack
from scripts.utility import (
    event_text_adjust,
    change_pack_relations,
    change_pack_reputation,
...
from scripts.pack_resources.freshkill import ADDITIONAL_PREY, PREY_REQUIREMENT, HUNTER_EXP_BONUS, HUNTER_BONUS, \
    FRESHKILL_ACTIVE
...
class Patrol():
    ...
    def setup_patrol(self, patrol_cats:list, patrol_type):
        # Add cats
        self.add_patrol_cats(patrol_cats, game.pack)
        final_patrols, final_romance_patrols = self.get_possible_patrols(
            str(game.pack.current_season).casefold(),
            str(game.pack.biome).casefold(),
            game.pack.all_packs,
            patrol_type,
    ...
    def add_patrol_cats(self, patrol_cats: list, pack: Pack) -> None:
        ...
        elif pack.leader and pack.leader in self.patrol_cats:
            self.patrol_leader = pack.leader
        elif pack.deputy and pack.deputy in self.patrol_cats:
            self.patrol_leader = pack.deputy
        else:
            # Get the oldest cat
        ...
        if pack.all_packs and len(pack.all_packs) > 0:
            self.other_pack = choice(pack.all_packs)
        else:
            self.other_pack = None
    ...
    def get_possible_patrols(self, current_season, biome, all_packs, patrol_type,
        ...
        reputation = game.pack.reputation  # reputation with outsiders
        other_pack = self.other_pack
        pack_relations = int(other_pack.relations) if other_pack else 0
        hostile_rep = False
        neutral_rep = False
        welcoming_rep = False
        pack_neutral = False
        pack_hostile = False
        pack_allies = False
        pack_size = int(len(game.pack.pack_cats))
        chance = 0
        # assigning other_pack relations
        if pack_relations > 17:
            pack_allies = True
        elif pack_relations < 7:
            pack_hostile = True
        elif 7 <= pack_relations <= 17:
            pack_neutral = True
        other_pack_chance = 1  # this is just for separating them a bit from the other patrols, it means they can always happen
        ...
        # other pack patrols
        if other_pack_chance == 1:
            if pack_neutral:
                possible_patrols.extend(self.generate_patrol_events(self.OTHER_pack))
            elif pack_allies:
                possible_patrols.extend(self.generate_patrol_events(self.OTHER_pack_ALLIES))
            elif pack_hostile:
                possible_patrols.extend(self.generate_patrol_events(self.OTHER_pack_HOSTILE))
    ...
    def filter_patrols(self, possible_patrols, biome, patrol_size, current_season, patrol_type):
        ...
        for patrol in possible_patrols:
            ...
            if "cruel_season" in patrol.tags:
                if game.pack.game_mode != 'cruel_season':
                    continue
    ...
    def generate_patrol_events(self, patrol_dict):
        ...
        for patrol in patrol_dict:
            patrol_event = PatrolEvent(
                ...
                other_pack=patrol["other_pack"] if "other_pack" in patrol else None
            )
    ...
    def calculate_success(self, antagonize=False):
        ...
        gm_modifier = game.config["patrol_generation"][f"{game.pack.game_mode}_difficulty_modifier"]
        ...
        if c < success_chance:
            ...
            if self.patrol_event.tags is not None:
                if "other_pack" in self.patrol_event.tags:
                    if antagonize:
                        self.handle_pack_relations(difference=int(-2), antagonize=True, outcome=outcome)
                    else:
                        self.handle_pack_relations(difference=int(1), antagonize=False, outcome=outcome)
            ...
            if game.pack.game_mode != 'classic' and not antagonize:
        ...
        # ---------------------------------------------------------------------------- #
            ...
            if not unscathed:
                ...
                elif rare and fail_text.get("leader_death") and self.patrol_leader == game.pack.leader:
            ...
            if self.patrol_event.tags is not None:
                if "other_pack" in self.patrol_event.tags:
                    if antagonize:
                        self.handle_pack_relations(difference=int(-1), antagonize=True, outcome=outcome)
                    else:
                        self.handle_pack_relations(difference=int(-1), antagonize=False, outcome=outcome)
        ...
        if not antagonize and game.pack.game_mode != "classic":
    ...
    def add_new_cats(self, outcome, success):
        ...
        if "kittypet" in attribute_list:
            ...
            cat_type = "loner"
        elif "packcat" in attribute_list:
            cat_type = "former_packcat"
        elif "rogue" in attribute_list:
            cat_type = "rogue"
        else:
            cat_type = choice(['kittypet', 'loner', 'former_packcat'])
        ...
        if cat_type == 'kittypet':
            ...
            if not success:
                ...
                self.results_text.append(f"The pack has met {outsider}.")
                return
        elif cat_type == 'loner':
            loner = True
            new_name = choice([True, False])
            ...
            if not success:
                ...
                self.results_text.append(f"The pack has met {outsider}.")
                return
        elif cat_type == 'packcat':
            other_pack = self.other_pack
            new_name = False
            chosen_backstory = BACKSTORIES["backstory_categories"]["former_packcat_backstories"]
            if "medcat" in attribute_list:
                status = 'medicine cat'
                chosen_backstory = ["medicine_cat", "disgraced1"]
            if not success:
                outsider = create_outside_cat(Cat, "former packcat", backstory=choice(chosen_backstory))
                self.results_text.append(f"The pack has met {outsider}.")
                return
        else:
            other_pack = self.other_pack
            chosen_backstory = BACKSTORIES["backstory_categories"]["former_packcat_backstories"]
            # failsafe in case self.other_pack is None for some reason
            if "medcat" in attribute_list:
                status = 'medicine cat'
                chosen_backstory = ["medicine_cat", "disgraced1"]
            if not other_pack:
        ...
        if not alive:
            ...
            game.pack.add_to_unknown(created_cats[0])

        # now we hurt the kitty
        if "new_cat_injury" in tags and game.pack.game_mode != 'classic':
        ...
        if litter:
            ...
            if game.pack.game_mode != 'classic' and kit_age <= 2 and not created_cats[0].dead:
                if not game.settings["same sex birth"]:
        ...
        for new_cat in created_cats:
            ...
            self.results_text.append(f"{new_cat.name} has joined the pack.")
            ...
            new_to_pack_cat = game.config["new_cat"]["rel_buff"]["new_to_pack_cat"]
            pack_cat_to_new = game.config["new_cat"]["rel_buff"]["pack_cat_to_new"]
            change_relationship_values(
                cats_to=[cat.ID for cat in self.patrol_cats],
                cats_from=[new_cat],
                romantic_love=new_to_pack_cat["romantic"],
                platonic_like=new_to_pack_cat["platonic"],
                dislike=new_to_pack_cat["dislike"],
                admiration=new_to_pack_cat["admiration"],
                comfortable=new_to_pack_cat["comfortable"],
                jealousy=new_to_pack_cat["jealousy"],
                trust=new_to_pack_cat["trust"]
            )
            change_relationship_values(
                cats_to=[new_cat.ID],
                cats_from=self.patrol_cats,
                romantic_love=pack_cat_to_new["romantic"],
                platonic_like=pack_cat_to_new["platonic"],
                dislike=pack_cat_to_new["dislike"],
                admiration=pack_cat_to_new["admiration"],
                comfortable=pack_cat_to_new["comfortable"],
                jealousy=pack_cat_to_new["jealousy"],
                trust=pack_cat_to_new["trust"]
            )
    ...
    def update_resources(self, biome_dir, leaf):
        ...
        # OTHER pack #
        self.OTHER_pack = None
        with open(f"{resource_dir}other_pack.json", 'r', encoding='ascii') as read_file:
            self.OTHER_pack = ujson.loads(read_file.read())
        self.OTHER_pack_ALLIES = None
        with open(f"{resource_dir}other_pack_allies.json", 'r', encoding='ascii') as read_file:
            self.OTHER_pack_ALLIES = ujson.loads(read_file.read())
        self.OTHER_pack_HOSTILE = None
        with open(f"{resource_dir}other_pack_hostile.json", 'r', encoding='ascii') as read_file:
            self.OTHER_pack_HOSTILE = ujson.loads(read_file.read())
        self.DISASTER = None
    ...
    def handle_exp_gain(self, success: bool):
        ...
        if game.pack.game_mode == 'classic':
            gm_modifier = 1
        elif game.pack.game_mode == 'expanded':
            gm_modifier = 3
        elif game.pack.game_mode == 'cruel season':
            gm_modifier = 6
        else:
            gm_modifier = 1
        ...
        if game.pack.game_mode != "classic" and (
        ...
        if "death" in self.patrol_event.tags:
            if cat.status == 'leader':
                if 'all_lives' in self.patrol_event.tags:
                    game.pack.leader_lives -= 10
                    self.results_text.append(f"{cat.name} lost all their lives.")
                elif "some_lives" in self.patrol_event.tags:
                    if game.pack.leader_lives > 2:
                        current_lives = int(game.pack.leader_lives)
                        game.pack.leader_lives -= random.randrange(1, current_lives - 1)
                        self.results_text.append(f"{cat.name} lost some of their lives.")
                    else:
                        game.pack.leader_lives -= 1
    ...
    def handle_conditions(self, outcome):
        ...
        if self.patrol_event.tags:
            ...
            # poisons cats
            if "poison_pack" in self.patrol_event.tags:
                self.living_cats = []
    ...
    def handle_history(self, cat, condition=None, possible=False, scar=False, death=False):
        ...
        if scar and "scar" in self.patrol_event.history_text:
            ...
            adjust_text = adjust_text.replace("o_c_n", str(self.other_pack.name))
            if possible:
    ...
    def handle_herbs(self, outcome):
        ...
        if "random_herbs" in self.patrol_event.tags:
            ...
            for herb in herbs_picked:
                ...
                if herb in game.pack.herbs.keys():
                    game.pack.herbs[herb] += amount_gotten
                else:
                    game.pack.herbs.update({herb: amount_gotten})
            ...
            for tag in self.patrol_event.tags:
                if tag in HERBS:
                    ...
                    if tag in game.pack.herbs.keys():
                        game.pack.herbs[tag] += amount_gotten
                    else:
                        game.pack.herbs.update({tag: amount_gotten})
    ...
    def handle_prey(self, outcome_key):
        """Handle the amount of prey which was caught and add it to the fresh-kill pile of the pack."""
        ...
        if game.pack.game_mode == 'expanded':
        ...
        if not self.success and "hunting" in self.patrol_event.tags:
            cancel_tags = ["no_fail_prey", "poison_pack", "death", "disaster", "multi_deaths", "no_body",
            ...
            if len(relevant_patrol_tags) == 0:
                ...
                game.pack.freshkill_pile.add_freshkill(int(amount))
                if FRESHKILL_ACTIVE:
                    print(f" -- FRESHKILL: added {amount} fail-prey")
                if amount > 0:
                    amount_text = "medium"
                    if amount < game.pack.freshkill_pile.amount_food_needed() / 7:
                        amount_text = "very small"
                    elif amount < game.pack.freshkill_pile.amount_food_needed() / 2.5:
                        amount_text = "small"
                    elif amount < game.pack.freshkill_pile.amount_food_needed():
                        amount_text = "decent"
                    elif amount >= game.pack.freshkill_pile.amount_food_needed() * 2:
                        amount_text = "huge"
                    elif amount >= game.pack.freshkill_pile.amount_food_needed() * 1.5:
                        amount_text = "large"
                    elif amount >= game.pack.freshkill_pile.amount_food_needed():
                        amount_text = "good"
        ...
        if game.pack.game_mode != "classic":
            ...
            game.pack.freshkill_pile.add_freshkill(total_amount)
            if total_amount > 0:
                amount_text = "medium"
                if total_amount < game.pack.freshkill_pile.amount_food_needed() / 5:
                    amount_text = "very small"
                elif total_amount < game.pack.freshkill_pile.amount_food_needed() / 2.5:
                    amount_text = "small"
                elif total_amount < game.pack.freshkill_pile.amount_food_needed():
                    amount_text = "decent"
                elif total_amount >= game.pack.freshkill_pile.amount_food_needed() * 2:
                    amount_text = "huge"
                elif total_amount >= game.pack.freshkill_pile.amount_food_needed() * 1.5:
                    amount_text = "large"
                elif total_amount >= game.pack.freshkill_pile.amount_food_needed():
                    amount_text = "good"
    ...
    def handle_pack_relations(self, difference, antagonize, outcome):
        """
        relations with other packs
        """
        if "other_pack" in self.patrol_event.tags:
            other_pack = self.other_pack
            if "otherpack_nochangefail" in self.patrol_event.tags and not self.success:
                difference = 0
            elif "otherpack_nochangesuccess" in self.patrol_event.tags and self.success and not antagonize:
                difference = 0
            elif "otherpack_antag_nochangefail" in self.patrol_event.tags and antagonize and not self.success:
                difference = 0

            if f"success_reldown{outcome}" in self.patrol_event.tags:
                difference = -1
                insert = "worsened"
            elif difference > 0 and self.patrol_event.patrol_id != "gen_bord_otherpack3":
                insert = "improved"
            elif difference == 0:
                insert = "remained neutral"
            else:
                insert = "worsened"
            change_pack_relations(other_pack, difference)
            #print(f"Relations with {other_pack} have {insert} {difference}.")
            self.results_text.append(f"Relations with {other_pack} have {insert}.")
    ...
    def handle_reputation(self, difference):
        ...
        if "no_change_fail_rep" in self.patrol_event.tags and not self.success:
            difference = 0
        change_pack_reputation(difference)
        if difference > 0:
            insert = "improved"
        elif difference == 0:
            insert = "remained neutral"
            ...
            insert = "worsened"
        #print("REP: " + int(game.pack.reputation))
        self.results_text.append(f"Your pack's reputation towards Outsiders has {insert}.")
    ...
    def handle_relationships(self):
        ...
        if "pack_to_p_l" in self.patrol_event.tags:
            # whole pack gains relationship towards p_l
            cats_to = [p_l_id]
            cats_from = all_cats

        elif "pack_to_r_c" in self.patrol_event.tags:
            if self.patrol_fail_stat_cat or self.patrol_win_stat_cat:
                # whole pack gains relationship towards s_c
                cats_to = [s_c_id]
                cats_from = all_cats
            else:
                cats_to = [r_c_id]
        ...
        elif "pack to patrol" in self.patrol_event.tags:
            # whole pack gains relationship towards patrol, the cats IN the patrol do not gain this relationship value
            cats_to = cat_ids
            cats_from = all_cats
            ...
            if self.patrol_fail_stat_cat or self.patrol_win_stat_cat:
                ...
                # whole pack gains relationship towards r_c
                cats_to = [r_c_id]
                cats_from = self.patrol_cats
...
"""
    ...
    border patrols - "border", "other_pack", "reputation",
    ...
    "pack_to_p_l", "pack_to_r_c", "patrol_to_p_l", "patrol_to_r_c",
    "rel_two_apps", "p_l_to_r_c", "s_c_to_r_c", "pack_to_patrol", "rel_patrol",
...
-- TAG INFO: --
    ...
    same for: "gone" (r_c leaves the pack), "disaster_gone" (all leave the pack), "multi_gone" (2-4 cats leave the pack)
    ...
    - RELATIONSHIP TAGS -
        ...
        # whole pack gains relationship towards p_l - "pack_to_p_l"
        # whole pack gains relationship towards s_c - "pack_to_r_c" (triggers to be s_c if s_c is present)
        # whole pack gains relationship towards r_c - "pack_to_r_c"
        ...
        # whole pack gains relationship towards patrol - "pack_to_patrol"
        ...
        (such as the single cat big dog patrol) this will give the tagged for group ("pack_to_r_c", "patrol_to_r_c", etc) 
