...
def medical_cats_condition_fulfilled(all_cats,
                                     amount_per_med,
                                     give_packmembers_covered=False):
    """
    returns True if the player has enough meds for the whole pack

    set give_packmembers_covered to True to return the int of packmembers that the meds can treat
    """
    ...
    if give_packmembers_covered is True:
        return can_care_for
...
def get_amount_cat_for_one_medic(pack):
    """Returns """
    amount = 10
    if pack and pack.game_mode == 'cruel season':
        amount = 7
    return amount
