...
from scripts.utility import event_text_adjust, change_pack_relations, change_relationship_values, get_alive_kits, \
    history_text_adjust
...
class Death_Events():
    ...
    def handle_deaths(self, cat, other_cat, war, enemy_pack, alive_kits, murder=False):
        """ 
        ...
        final_events = self.generate_events.filter_possible_short_events(possible_short_events, cat, other_cat, war,
                                                                         enemy_pack,
                                                                         other_pack, alive_kits, murder=murder)
        ...
        try:
            ...
            return
        death_text = event_text_adjust(Cat, death_cause.event_text, cat, other_cat, other_pack_name)
        ...
        if murder:
            ...
            death_history = history_text_adjust(death_history, other_pack_name, game.pack)
            if murder_unrevealed_history:
                murder_unrevealed_history = history_text_adjust(murder_unrevealed_history, other_pack_name, game.pack)
        ...
        if cat.status == 'leader':
            if "all_lives" in death_cause.tags:
                ...
                game.pack.leader_lives -= random.randrange(2, current_lives - 1)
                additional_event_text += cat.die(body)
            else:
        ...
        if "multi_death" in death_cause.tags:
            if other_cat.status == 'leader':
                if "all_lives" in death_cause.tags:
                    ...
                    game.pack.leader_lives -= 1
                    additional_event_text += other_cat.die(body)
                other_death_history = history_text_adjust(death_cause.history_text.get('lead_death'), other_pack_name, game.pack)

            else:
                additional_event_text += other_cat.die(body)
                other_death_history = history_text_adjust(death_cause.history_text.get('reg_death'), other_pack_name, game.pack)
        ...
        # handle relationships with other packs
        ...
        types = ["birth_death"]
        if "other_pack" in death_cause.tags:
