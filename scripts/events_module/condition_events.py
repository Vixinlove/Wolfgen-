...
from scripts.utility import event_text_adjust, get_med_cats, change_relationship_values, change_pack_relations, \
    history_text_adjust
...
class Condition_Events():
    ...
    def handle_illnesses(self, cat, season=None):
        ...
        triggered = False
        if cat.dead or game.pack.game_mode == "classic":
            if cat.dead:
                triggered = True
            return triggered
        ...
        if cat.is_ill():
            ...
            # ---------------------------------------------------------------------------- #
            random_number = int(
                random.random() * game.get_config_value("condition_related", f"{game.pack.game_mode}_illness_chance"))
    ...
    def handle_injuries(self, cat, other_cat=None, alive_kits=None, war=None, enemy_pack=None, season=None):
        """ 
        ...
        """
        has_other_pack = False
        triggered = False
        text = None
        random_number = int(random.random() * game.get_config_value("condition_related", f"{game.pack.game_mode}_injury_chance"))
        ...
        if cat.is_injured() and game.pack.game_mode != 'classic':
            ...
            if triggered:
                if war:
                    other_pack = enemy_pack
                else:
                    other_pack = random.choice(game.pack.all_packs)
                if other_pack:
                    other_pack_name = f'{other_pack.name}Pack'

                if other_pack_name == 'None':
                    other_pack = game.pack.all_packs[0]
                    other_pack_name = f'{other_pack.name}Pack'
                ...
                final_events = self.generate_events.filter_possible_short_events(possible_events, cat, other_cat, war,
                                                                                 enemy_pack, other_pack, alive_kits)

                if len(final_events) > 0:
                    injury_event = random.choice(final_events)

                    if "other_pack" in injury_event.tags or "war" in injury_event.tags:
                        has_other_pack = True

                    if "rel_up" in injury_event.tags:
                        change_pack_relations(other_pack, difference=1)
                    ...
                    text = event_text_adjust(Cat, injury_event.event_text, cat, other_cat, other_pack_name)
                        ...
                        if injury_event.history_text:
                            ...
                            if "scar" in injury_event.history_text:
                                possible_scar = history_text_adjust(injury_event.history_text['scar'],
                                                                   other_pack_name, game.pack, other_cat_rc = other_cat)
    ...
    def handle_already_ill(self, cat):
        ...
        starting_life_count = game.pack.leader_lives
        ...
        for illness in illnesses:
            ...
            elif cat.status == 'leader' and starting_life_count != game.pack.leader_lives:
    ...
    def handle_already_injured(self, cat):
        ...
        for injury in injuries:
            ...
            if cat.dead or (cat.status == 'leader' and starting_life_count != game.pack.leader_lives):
                triggered = True
            ...
            for condition in cat.permanent_condition:
                if cat.permanent_condition[condition]['severity'] in ['major', 'severe']:
                    ...
                    if not int(random.random() * chance):
                        ...
                        elif game.pack.leader is not None:
                            if not game.pack.leader.dead and not game.pack.leader.exiled and \
                                    not game.pack.leader.outside and cat.moons < 120:
                                retire_involved.append(game.pack.leader.ID)
                                event = f"{game.pack.leader.name}, seeing {cat.name} struggling the last few moons " \
                        ...
                        if cat.age == 'adolescent':
                            event += f" They are given the name {cat.name.prefix}{cat.name.suffix} in honor " \
                                     f"of their contributions to {game.pack.name}Pack."
    ...
    def give_risks(self, cat, event_list, condition, progression, conditions, dictionary):
        ...
        for risk in conditions[condition]["risks"]:
            ...
            if medical_cats_condition_fulfilled(Cat.all_cats.values(),
                                                get_amount_cat_for_one_medic(game.pack)):
                chance += 10  # lower risk if enough meds
            if game.pack.medicine_cat is None and chance != 0:
    ...
    def use_herbs(self, cat, condition, conditions, source):
        ...
        try:
            ...
            print(f"WARNING: {condition} does not exist in it's condition dict! if the condition is 'thorn in paw' or "
                  ...
                  "this as a bug.")
            return
        herb_set = pack_herbs.intersection(needed_herbs)
        ...
        if usable_herbs:
            ...
            while game.pack.herbs[herb_used] <= 0:
                print(f"Warning: {herb_used} was chosen to use, although you currently have "
                      f"{game.pack.herbs[herb_used]}. Removing {herb_used} from herb dict, finding a new herb...")
            ...
            # deplete the herb
            amount_used = 1
            game.pack.herbs[herb_used] -= amount_used
