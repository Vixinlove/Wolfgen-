...
class GenerateEvents:
        ...
        if file_path in GenerateEvents.loaded_events:
            ...
            for event in events_dict:
                ...
                event = ShortEvent(
                    ...
                    other_pack=event["other_pack"] if "other_pack" in event else False,
    ...
    def possible_short_events(self, cat_type=None, age=None, event_type=None):
        ...
        if cat_type not in game.pack.CAT_TYPES:
            print(
                f"WARNING: unrecognised cat status {cat_type} in generate_events. Have you added it to CAT_TYPES in "
                f"pack.py?")

        elif game.pack.biome not in game.pack.BIOME_TYPES:
            print(
                f"WARNING: unrecognised biome {game.pack.biome} in generate_events. Have you added it to BIOME_TYPES "
                f"in pack.py?")
    ...
    def filter_possible_short_events(self, possible_events, cat, other_cat, war, enemy_pack, other_pack, alive_kits, murder=False, murder_reveal=False):
        final_events = []
        ...
        if war and random.randint(1, 10) != 1 and other_pack == enemy_pack:
            war_event = True
        else:
            war_event = False
        ...
        for event in possible_events:
            ...
            if game.pack.game_mode in ["expanded", "cruel season"] and "classic" in event.tags:
                continue
            ...
            if "some_lives" in event.tags and game.pack.leader_lives <= 3:
                continue

            if "low_lives" in event.tags:
                if game.pack.leader_lives > 3:
                    continue

            # check season
            if game.pack.current_season not in event.tags:
                continue
            ...
            if "other_pack" in event.tags:
                if "war" in event.tags and not war:
                    continue
                if "ally" in event.tags and int(other_pack.relations) < 17:
                    continue
                elif "neutral" in event.tags and (int(other_pack.relations) <= 7 or int(other_pack.relations) >= 17):
                    continue
                elif "hostile" in event.tags and int(other_pack.relations) > 7:
                    continue

            # check if pack has kits
            if "pack_kits" in event.tags and not alive_kits:
                continue
    ...
    def possible_ongoing_events(self, event_type=None, specific_event=None):
        ...
        if game.pack.biome not in game.pack.BIOME_TYPES:
            print(
                f"WARNING: unrecognised biome {game.pack.biome} in generate_events. Have you added it to BIOME_TYPES in pack.py?")
...
"war" < event only happens during war and ensures o_c in event is warring pack
"other_pack" < mark event as including another pack
"rel_down" < event decreases relation with other pack
"rel_up" < event increases relation with other pack
"hostile" < event only happens with hostile packs
"neutral" < event only happens with neutral packs
"ally" < event only happens with allied packs
...
"pack_kits" < pack must have kits for this event to appear
...
"parent" < this litter or kit also comes with a parent (this does not include adoptive parents from within the pack)
