...
class Welcoming_Events():
    """All events which are related to welcome a new cat in the pack."""

    def __init__(self) -> None:
        pass
    
    def welcome_cat(self, pack_cat: Cat, new_cat: Cat) -> None:
        """Checks and triggers the welcome event from the pack cat to the new cat.
            ...
            -------
        """
        if new_cat.ID == pack_cat.ID:
            return

        # setup the status as "key" to use it
        status = pack_cat.status
        ...
        interaction_str = event_text_adjust(Cat, interaction_str, pack_cat, new_cat)
        ...
        game.cur_events_list.append(Single_Event(
            interaction_str, ["relation", "interaction"], [new_cat.ID, pack_cat.ID]))

        # add to relationship logs
        if new_cat.ID in pack_cat.relationships:
            if pack_cat.age == 1:
                pack_cat.relationships[new_cat.ID].log.append(interaction_str + f" - {pack_cat.name} was {pack_cat.moons} moons old")
            else:
                pack_cat.relationships[new_cat.ID].log.append(interaction_str + f" - {pack_cat.name} was {pack_cat.moons} moons old")

            new_cat.relationships[pack_cat.ID].link_relationship()

        if pack_cat.ID in new_cat.relationships:
            if new_cat.age == 1:
                new_cat.relationships[pack_cat.ID].log.append(interaction_str + f" - {new_cat.name} was {new_cat.moons} moon old")
            else:
                new_cat.relationships[pack_cat.ID].log.append(interaction_str + f" - {new_cat.name} was {new_cat.moons} moons old")
