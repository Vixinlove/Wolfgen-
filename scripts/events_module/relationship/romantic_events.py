...
class Romantic_Events():
    ...
    def start_interaction(self, cat_from, cat_to):
        ...
        _season = [str(game.pack.current_season).casefold(), "Any", "any"]
        _biome = [str(game.pack.biome).casefold(), "Any", "any"]
        for interaction in possible_interactions:
            ...
            cat_fulfill = cats_fulfill_single_interaction_constraints(cat_from, cat_to, interaction, game.pack.game_mode)
            if not cat_fulfill:
                continue
        ...
        if len(chosen_interaction.get_injuries) > 0 and game.pack.game_mode != 'classic':
    ...
    def handle_breakup(self, relationship_from, relationship_to, cat_from, cat_to):
        ...
        from_mate_in_pack = False
        if cat_to.ID in cat_from.mate:
            from_mate_in_pack = cat_to.is_alive() and not cat_to.outside
        else:
            return

        if not self.had_one_event and relationship_from.mates and from_mate_in_pack:
    ...
    def handle_confession(self, cat_from):
        ...
        alive_inpack_from_mates = [mate for mate in cat_from.mate if not cat_from.fetch_cat(mate).dead and not cat_from.fetch_cat(mate).outside]
        alive_inpack_to_mates = [mate for mate in cat_to.mate if not cat_to.fetch_cat(mate).dead and not cat_to.fetch_cat(mate).outside]
        poly = len(alive_inpack_from_mates) > 0 or len(alive_inpack_to_mates) > 0
    ...
    def current_mates_allow_new_mate(self, cat_from, cat_to) -> bool:
        ...
        alive_inpack_from_mates = [mate for mate in cat_from.mate if not cat_from.fetch_cat(mate).dead and not cat_from.fetch_cat(mate).outside]
        if len(alive_inpack_from_mates) > 0:
            for mate_id in alive_inpack_from_mates:
        ...
        all_mates_fulfill_current_to_new = True
        alive_inpack_to_mates = [mate for mate in cat_to.mate if not cat_to.fetch_cat(mate).dead and not cat_to.fetch_cat(mate).outside]
        if len(alive_inpack_to_mates) > 0:
            for mate_id in alive_inpack_to_mates:
    ...
    def get_mate_string(self, key, poly, cat_from, cat_to):
        ...
        if not poly:
            ...
            alive_inpack_from_mates = [mate for mate in cat_from.mate if not cat_from.fetch_cat(mate).dead and not cat_from.fetch_cat(mate).outside]
            alive_inpack_to_mates = [mate for mate in cat_to.mate if not cat_to.fetch_cat(mate).dead and not cat_to.fetch_cat(mate).outside]
            if len(alive_inpack_from_mates) > 0 and len(alive_inpack_to_mates) > 0:
                poly_key = "both_mates"
            elif len(alive_inpack_from_mates) > 0 and len(alive_inpack_to_mates) <= 0:
                poly_key = "m_c_mates"
            elif len(alive_inpack_from_mates) <= 0 and len(alive_inpack_to_mates) > 0:
                poly_key = "r_c_mates"
