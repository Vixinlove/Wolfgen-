...
class Pregnancy_Events():
    ...
    def set_biggest_family(self):
        """Gets the biggest family of the pack."""
        biggest_family = None
    ...
    def handle_pregnancy_age(self, pack):
        """Increase the moon for each pregnancy in the pregnancy dictionary"""
        for pregnancy_key in pack.pregnancy_data.keys():
            pack.pregnancy_data[pregnancy_key]["moons"] += 1

    def handle_having_kits(self, cat, pack):
        ...
        if cat.ID in pack.pregnancy_data:
            moons = pack.pregnancy_data[cat.ID]["moons"]
            if moons == 1:
                self.handle_one_moon_pregnant(cat, pack)
                return
            if moons >= 2:
                self.handle_two_moon_pregnant(cat, pack)
        ...
        # check if there is a cat in the pack for the second parent
        ...
        if not int(random.random() * chance):
            ...
            if kits_are_adopted:
                self.handle_adoption(cat, second_parent, pack)
            else:
                self.handle_zero_moon_pregnant(cat, second_parent, second_parent_relation, pack)

    # ---------------------------------------------------------------------------- #
    #                                 handle events                                #
    # ---------------------------------------------------------------------------- #

    def handle_adoption(self, cat: Cat, other_cat=None, pack=game.pack):
        ...
        if other_cat and other_cat.ID in pack.pregnancy_data:
            return
        ...
        kits = self.get_kits(amount, None, None, pack, adoptive_parents=adoptive_parents)
    ...
    def handle_zero_moon_pregnant(self, cat: Cat, other_cat=None, relation=None, pack=game.pack):
        ...
        if not other_cat and cat.gender == 'male':
            amount = self.get_amount_of_kits(cat)
            kits = self.get_kits(amount, cat, None, pack)
            insert = 'this should not display'
            if amount == 1:
                insert = 'a single kitten'
            if amount > 1:
        ...
        pack.pregnancy_data[pregnant_cat.ID] = {
        ...
        text = event_text_adjust(Cat, text, pregnant_cat, pack=pack)
        game.cur_events_list.append(Single_Event(text, "birth_death", pregnant_cat.ID))

    def handle_one_moon_pregnant(self, cat: Cat, pack=game.pack):
        """Handles if the cat is one moon pregnant."""
        if cat.ID not in pack.pregnancy_data.keys():
            return

        # if the pregnant cat killed meanwhile, delete it from the dictionary
        if cat.dead:
            del pack.pregnancy_data[cat.ID]
            return
        ...
        pack.pregnancy_data[cat.ID]["amount"] = amount

        # if the cat is outside of the pack, they won't guess how many kits they will have
        if cat.outside:
            return
        ...
        text = event_text_adjust(Cat, text, cat, pack=pack)
        game.cur_events_list.append(Single_Event(text, "birth_death", cat.ID))

    def handle_two_moon_pregnant(self, cat: Cat, pack=game.pack):
        ...
        kits_amount = pack.pregnancy_data[cat.ID]["amount"]
        other_cat_id = pack.pregnancy_data[cat.ID]["second_parent"]
        other_cat = Cat.all_cats.get(other_cat_id)

        kits = self.get_kits(kits_amount, cat, other_cat, pack)
        ...
        if not cat.outside and other_cat is None:
            ...
            if other_cat and not other_cat.outside:
                adding_text = choice(events["birth"]["outside_in_pack"])
        ...
        if not int(random.random() * death_chance):  # chance for a cat to die during childbirth
        ...
        elif pack.game_mode != 'classic' and not cat.outside:  # if cat doesn't die, give recovering from birth
            ...
            if 'blood loss' in cat.injuries:
                ...
                event_list.append(choice(possible_events))
        if pack.game_mode != 'classic' and not cat.dead: 
        ...
        print_event = event_text_adjust(Cat, print_event, cat, other_cat, pack=pack)
        # display event
    ...
    def get_kits(self, kits_amount, cat=None, other_cat=None, pack=game.pack, adoptive_parents=None):
        ...
        if cat and cat.gender == 'female':
            backstory = choice(['halfpack1', 'outsider_roots1'])
        elif cat:
            backstory = choice(['halfpack2', 'outsider_roots2'])
        else: # cat is adopted
        ...
        for kit in range(kits_amount):
            ...
            # don't delete the game.pack condition, this is needed for a test
            if game.pack and not int(random.random() * game.config["cat_generation"]["base_permanent_condition"]) \
                    and game.pack.game_mode != 'classic':
            ...
            #### GIVE HISTORY ###### 
            self.history.add_beginning(kit, pack_born=bool(cat))
        ...
        if blood_parent:
            ...
            pack.unknown_cats.append(blood_parent.ID)
    ...
    def get_balanced_kit_chance(self, first_parent: Cat, second_parent: Cat, affair) -> int:
        ...
        # CURRENT CAT AMOUNT
        # - increase the inverse chance if the pack is bigger
        living_cats = len([i for i in Cat.all_cats.values() if not (i.dead or i.outside or i.exiled)])
        if living_cats < 10:
        ...
        # AGE
        # - decrease the inverse chance if the whole pack is really old
