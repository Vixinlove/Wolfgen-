...
from scripts.utility import event_text_adjust, change_pack_relations, change_relationship_values, create_new_cat
...
class NewCatEvents:
    ...
    def handle_new_cats(self, cat: Cat, other_cat, war, enemy_pack, alive_kits):
        """ 
        ...
        if self.has_outside_cat():
            if random.randint(1, 3) == 1:
                ...
                game.pack.add_to_pack(outside_cat)
                history = History()
        ...
        if new_cat_event.litter:
            ...
            game.pack.add_to_unknown(blood_parent)
        ...
        if "injured" in new_cat_event.tags and game.pack.game_mode != "classic":
            major_injuries = []
        ...
        if "rel_down" in new_cat_event.tags:
            difference = -1
            change_pack_relations(other_pack, difference=difference)

        elif "rel_up" in new_cat_event.tags:
            difference = 1
            change_pack_relations(other_pack, difference=difference)

        event_text = event_text_adjust(Cat, new_cat_event.event_text, cat, other_cat, other_pack_name,
                                       new_cat=created_cats[0])

        types = ["misc"]
        if "other_pack" in new_cat_event.tags:
    ...
    def has_outside_cat(self):
        outside_cats = [i for i in Cat.all_cats.values() if i.status in ["kittypet", "loner", "rogue", "former packcat"] and not i.dead and i.outside]
        return any(outside_cats)

    def select_outside_cat(self):
        outside_cats = [i for i in Cat.all_cats.values() if i.status in ["kittypet", "loner", "rogue", "former packcat"] and not i.dead and i.outside]
        for cat in outside_cats:  # iterating over the generated list
            if cat.status in ["kittypet", "loner", "rogue", "former packcat"] and not cat.dead:
                return cat
