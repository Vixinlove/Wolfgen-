...
from scripts.utility import event_text_adjust, change_pack_relations, change_relationship_values, get_med_cats
...
class DisasterEvents():
    ...
    def handle_disasters(self):
        ...
        if game.pack.primary_disaster or game.pack.secondary_disaster:
            if game.pack.secondary_disaster:
                self.handle_current_secondary_disaster()
            if game.pack.primary_disaster:
        ...
        for event in possible_events:
            if event.priority == 'secondary':
                print('priority')
                continue
            if game.pack.current_season not in event.season:
                print('season')
                continue
            if (game.pack.camp_bg and 'any') not in event.camp:
                print('camp')
                continue
        ...
        game.pack.primary_disaster = chosen_disaster

        # display trigger event
        event = self.disaster_text(chosen_disaster.trigger_events)
        event.replace("c_n", f"{game.pack.name}Pack")
    ...
    def handle_current_primary_disaster(self):
        ...
        if not int(random.random() * 10):
            game.pack.primary_disaster.current_duration += 2
        else:
            game.pack.primary_disaster.current_duration += 1

        # triggering conclusion if duration reaches 0
        if game.pack.primary_disaster.current_duration >= game.pack.primary_disaster.duration:
            event = self.disaster_text(game.pack.primary_disaster.conclusion_events)
            game.cur_events_list.append(
                Single_Event(event, "misc"))
            game.pack.primary_disaster = None
            return
        else:
            # giving a progression event
            event_list = game.pack.primary_disaster.progress_events[f"moon{game.pack.primary_disaster.current_duration}"]
            event = self.disaster_text(event_list)
            game.cur_events_list.append(
                Single_Event(event, "misc"))

            # checking if a secondary disaster is triggered
            if game.pack.primary_disaster.secondary_disasters:
                print('secondary disaster rolling')
                picked_disasters = []
                for potential_disaster in game.pack.primary_disaster.secondary_disasters:
                    current_primary_duration = game.pack.primary_disaster.current_duration
                    default_primary_duration = game.pack.primary_disaster.duration
                ...
                if picked_disasters:
                    ...
                    game.pack.secondary_disaster = secondary_disaster
            return
    ...
    def handle_current_secondary_disaster(self):
        ...
        """
        if not int(random.random() * 10):
            game.pack.secondary_disaster.current_duration += 2
        else:
            game.pack.secondary_disaster.current_duration += 1

        # triggering conclusion if duration reaches 0
        if game.pack.secondary_disaster.current_duration >= game.pack.secondary_disaster.duration:
            event = self.disaster_text(game.pack.secondary_disaster.conclusion_events)
            game.cur_events_list.append(
                Single_Event(event, "misc"))
            game.pack.secondary_disaster = None
            return
        else:
            # giving a progression event
            event_list = game.pack.secondary_disaster.progress_events[f"moon{game.pack.secondary_disaster.current_duration}"]
            event = self.disaster_text(event_list)
            game.cur_events_list.append(
                Single_Event(event, "misc"))
    ...
    def disaster_text(self, text_list):
        ...
        leader = Cat.fetch_cat(game.pack.leader)
        deputy = Cat.fetch_cat(game.pack.deputy)
        ...
        # removing events that mention ranks if those ranks are not currently filled in the pack
        ...
        text = text.replace("c_n", f"{game.pack.name}Pack")
