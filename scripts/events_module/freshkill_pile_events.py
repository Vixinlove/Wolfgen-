...
from scripts.pack_resources.freshkill import Freshkill_Pile, MAL_PERCENTAGE , STARV_PERCENTAGE, FRESHKILL_ACTIVE, FRESHKILL_EVENT_TRIGGER_FACTOR, FRESHKILL_EVENT_ACTIVE
...
class Freshkill_Events():
    ...
    def handle_nutrient(self, cat: Cat, nutrition_info: dict) -> None:
        ...
        if cat_nutrition.percentage <= 0:
            ...
            death_text = event_text_adjust(Cat, chosen_event.event_text, cat, other_cat, other_pack_name)
            ...
            if cat.status != "leader" and chosen_event.history_text[0] is not None:
                history_text = event_text_adjust(Cat, chosen_event.history_text[0], cat, other_cat, other_pack_name)
            elif cat.status == "leader" and chosen_event.history_text[1] is not None:
                history_text = event_text_adjust(Cat, chosen_event.history_text[1], cat, other_cat, other_pack_name)
        ...
        chosen_event = (random.choice(final_events))
        event_text = event_text_adjust(Cat, chosen_event.event_text, cat, other_cat, other_pack_name)
        types = ["health"]
