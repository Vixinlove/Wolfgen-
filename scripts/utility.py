...
def get_alive_pack_queens(cat_cls):
    """
...
def get_alive_kits(Cat):
    """
    returns a list of IDs for all living kittens in the pack
    """
...
def get_med_cats(Cat, working=True):
    """
    returns a list of all meds and med apps currently alive, in the pack, and able to work
...
def get_living_pack_cat_count(Cat):
    """
    TODO: DOCS
    """
    count = 0
...
def get_current_season():
    ...
    if game.config['lock_season']:
        ...
        return game.pack.starting_season
    ...
    index = game.pack.age % 12 + modifiers[game.pack.starting_season]

    if index > 11:
        index = index - 12

    game.pack.current_season = game.pack.seasons[index]

    return game.pack.current_season

def change_pack_reputation(difference):
    """
    will change the pack's reputation with outsider cats according to the difference parameter.
    """
    game.pack.reputation += difference


def change_pack_relations(other_pack, difference):
    """
    will change the pack's relation with other packs according to the difference parameter.
    """
    # grab the pack that has been indicated
    other_pack = other_pack
    # grab the relation value for that pack
    y = game.pack.all_packs.index(other_pack)
    pack_relations = int(game.pack.all_packs[y].relations)
    # change the value
    pack_relations += difference
    # making sure it doesn't exceed the bounds
    if pack_relations > 30:
        pack_relations = 30
    elif pack_relations < 0:
        pack_relations = 0
    # setting it in the pack save
    game.pack.all_packs[y].relations = pack_relations
...
def create_new_cat(Cat,
    ...
    :param new_name: set True if cat(s) is a loner/rogue receiving a new pack name - default: False
    ...
    :param other_pack: if new cat(s) are from a neighboring pack, pass the name of their home pack - default: None
    ...
    :param outside: set this as True to generate the cat as an outsider instead of as part of the pack - default: False (pack cat)
    """
    accessory = None
    ...
    if backstory in (
            BACKSTORIES["backstory_categories"]["former_packcat_backstories"] or BACKSTORIES["backstory_categories"]["otherpack_categories"]):
        other_pack = True
    ...
    for index in range(number_of_cats):
        ...
        if other_pack or kit or litter or age < 12:
...
def create_outside_cat(Cat, status, backstory, alive=True, thought=None):
    ...
    if backstory in BACKSTORIES["backstory_categories"]["rogue_backstories"]:
        status = 'rogue'
    elif backstory in BACKSTORIES["backstory_categories"]["former_packcat_backstories"]:
    ...
    thought = "Wonders about those pack cats they just met"
    new_cat.thought = thought
    ...
    game.pack.add_cat(new_cat)
    game.pack.add_to_outside(new_cat)
...
def get_special_snippet_list(chosen_list, amount, sense_groups=None, return_string=True):
    ...
    """
    biome = game.pack.biome.casefold()
...
def history_text_adjust(text,
    ...
    """
    if "o_c" in text:
        text = text.replace("o_c", other_pack_name)
    if "c_n" in text:
        text = text.replace("c_n", pack.name)
    if "r_c" in text and other_cat_rc:
        text = text.replace("r_c", str(other_cat_rc.name))
    return text

def ongoing_event_text_adjust(Cat, text, pack=None, other_pack_name=None):
    """
    ...
    :param other_pack_name: the other pack's name if another pack is involved
    """
    cat_dict = {}
    if "lead_name" in text:
        kitty = Cat.fetch_cat(game.pack.leader)
        cat_dict["lead_name"] = (str(kitty.name), choice(kitty.pronouns))
    if "dep_name" in text:
        kitty = Cat.fetch_cat(game.pack.deputy)
    ...
    if pack:
        ...
        if game.pack is None:
            pack_name = game.switches["pack_list"][0]
        else:
            pack_name = str(game.pack.name)

    text = text.replace("c_n", pack_name + "pack")
...
def event_text_adjust(Cat,
    ...
    :param other_pack_name: The other pack involved in the event
    :param new_cat: The cat taking the place of n_c
    :param pack: The player's pack
...
def leader_ceremony_text_adjust(Cat,
    ...
    text = text.replace("c_n", str(game.pack.name) + "pack")
...
def ceremony_text_adjust(Cat,
                         ...
                         living_parents=(),
                         dead_parents=()):
    packname = str(game.pack.name + "pack")
    ...
    cat_dict = {
        ...
        "l_n": (str(game.pack.leader.name), choice(game.pack.leader.pronouns)) if game.pack.leader else (
            "leader_name", None),
        "c_n": (packname, None),
    }
...
def adjust_patrol_text(text, patrol):
    ...
    other_pack_name = patrol.other_pack.name
    s = 0
    for x in range(text.count('o_c_n')):
        if 'o_c_n' in text:
            for y in vowels:
                if str(other_pack_name).startswith(y):
                    modify = text.split()
                    pos = 0
                    if 'o_c_n' in modify:
    ...
    text = text.replace('o_c_n', str(other_pack_name) + 'pack')
    ...
    try:
        ...
        if cat.pelt.opacity <= 97 and not cat.prevent_fading and game.settings["fading"] and dead:
            ...
            if cat.df:
                ...
                temp = sprites.sprites['fadestarpack' + stage + cat_sprite].copy()
                temp.blit(new_sprite, (0, 0))
                new_sprite = temp
