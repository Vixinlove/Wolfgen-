...
class Cat():
    ...
    outside_cats: Dict[str, Cat] = {}  # cats outside the pack
    ...
    def __init__(self,
        ...
        if ID is None:
            ...
            if game.pack:
                faded_cats = game.pack.faded_ids
            else:
                faded_cats = []
    ...
    def die(self, body: bool = True):
        ...
        if game.pack.game_mode != 'classic' and not (self.outside or self.exiled):
            self.grief(body)
    ...
    def grief(self, body: bool):
        ...
        for cat in Cat.all_cats.values():
            ...
            if very_high_values:
                ...
                if not body_treated and "rosemary" in game.pack.herbs:  
                    body_treated = True
                    game.pack.herbs["rosemary"] -= 1
                    if game.pack.herbs["rosemary"] <= 0:
                        game.pack.herbs.pop("rosemary")
            ...
            # fail to minor grief. 
                ...
                minor_grief_messages = (
                            ...
                            "Bargins with Starpack, begging them to send r_c back",
    ...
    def gone(self):
        ...
        game.pack.add_to_outside(self)

    def add_to_pack(self) -> list:
        """ Makes a "outside cat" a pack cat. Returns a list of any additional cats that
            are coming with them. """
        self.outside = False

        game.pack.add_to_pack(self)

        # check if there are kits under 12 moons with this cat and also add them to the pack
    ...
    def status_change(self, new_status, resort=False):
        ...
        if self.status == 'apprentice':
            ...
            if old_status == 'leader':
                if game.pack.leader:
                    if game.pack.leader.ID == self.ID:
                        game.pack.leader = None
                        game.pack.leader_predecessors += 1

                    # don't remove the check for game.pack, this is needed for tests
            if game.pack and game.pack.deputy:
                if game.pack.deputy.ID == self.ID:
                    game.pack.deputy = None
                    game.pack.deputy_predecessors += 1

        elif self.status == 'medicine cat':
            if game.pack is not None:
                game.pack.new_medicine_cat(self)
    ...
    def load_history(self):
        try:
            if game.switches['pack_name'] != '':
                packname = game.switches['pack_name']
            else:
                packname = game.switches['pack_list'][0]
        except IndexError:
            print('WARNING: History failed to load, no pack in game.switches?')
            return

        history_directory = get_save_dir() + '/' + packname + '/history/'
    ...
    def generate_lead_ceremony(self):
        ...
        for intro in all_intros:
            ...
            if game.pack.age != 0 and "new_pack" in tags:
                continue
            elif game.pack.age == 0 and "new_pack" not in tags:
                continue
        ...
        for rel in relationships:
            ...
            if kitty and kitty.dead and kitty.status != 'newborn':
                ...
                if starpack:
                    if kitty.ID not in game.pack.starpack_cats:
                        continue
                else:
                    if kitty.ID not in game.pack.darkforest_cats:
                        continue
                # guides aren't allowed here
                if kitty == game.pack.instructor:
                    continue
                else:
        ...
        if len(life_givers) < 8:
            ...
            if starpack:
                ...
                possible_sc_cats = [i for i in game.pack.starpack_cats if
                ...
                #print(game.pack.darkforest_cats)
                possible_df_cats = [i for i in game.pack.darkforest_cats if
                                    self.fetch_cat(i) and
                                    i not in life_givers and
                                    self.fetch_cat(i).status not in ['leader', 'newborn']]
        ...
        if not life_giving_leader:
            ...
            if coin_flip == 1:
                ...
                ancient_leader = True
                if starpack:
                    for kitty in reversed(game.pack.starpack_cats):
                        if self.fetch_cat(kitty) and \
                            self.fetch_cat(kitty).status == 'leader':
                            life_giving_leader = kitty
                            break
                else:
                    for kitty in reversed(game.pack.darkforest_cats):
                        if self.fetch_cat(kitty) and \
                            self.fetch_cat(kitty).status == 'leader':
                            life_giving_leader = kitty
                            break
                ...
                if starpack:
                    for kitty in game.pack.starpack_cats:
                        if self.fetch_cat(kitty) and \
                            self.fetch_cat(kitty).status == 'leader':
                            life_giving_leader = kitty
                            break
                else:
                    for kitty in game.pack.darkforest_cats:
                        if self.fetch_cat(kitty) and \
                            self.fetch_cat(kitty).status == 'leader':
                            life_giving_leader = kitty
                            break
        ...
        for giver in life_givers:
            ...
            for life in possible_lives:
                ...
                if "guide" in tags and giver_cat != game.pack.instructor:
                    continue
                if game.pack.age != 0 and "new_pack" in tags:
                    continue
                elif game.pack.age == 0 and "new_pack" not in tags:
    ...
    def thoughts(self):
        ...
        try:
            season = game.pack.current_season
        except:
            season = None
        ...
        if where_kitty == 'inside':
            while other_cat == self.ID and len(all_cats) > 1 \
                ...
                if i > 100:
                    ...
                    break
        # for dead cats
        elif where_kitty in ['starpack', 'hell', 'UR']:
            while other_cat == self.ID and len(all_cats) > 1:
                other_cat = choice(list(all_cats.keys()))
                i += 1
                if i > 100:
    ...
    def moon_skip_illness(self, illness):
        ...
        if mortality and not int(random() * mortality):
            if self.status == "leader":
                ...
                if game.pack.leader_lives > 0:
                    ...
                    game.cur_events_list.append(Single_Event(text, ["birth_death", "health"], game.pack.leader.ID))
                elif game.pack.leader_lives <= 0:
        ...
        moons_with = game.pack.age - self.illnesses[illness]["moon_start"]
    ...
    def moon_skip_injury(self, injury):
        ...
        moons_with = game.pack.age - self.injuries[injury]["moon_start"]
    ...
    def get_ill(self, name, event_triggered=False, lethal=True, severity='default'):
        ...
        if game.pack.game_mode == "cruel season":
            if mortality != 0:
        ...
        if new_illness.name not in self.illnesses:
            self.illnesses[new_illness.name] = {
                ...
                "moon_start": game.pack.age if game.pack else 0,
    ...
    def get_injured(self, name, event_triggered=False, lethal=True, severity='default'):
        ...
        if medical_cats_condition_fulfilled(Cat.all_cats.values(), get_amount_cat_for_one_medic(game.pack)):
            duration = med_duration
        if severity != 'minor':
            duration += randrange(-1, 1)
        if duration == 0:
        ...
        if len(new_injury.also_got) > 0 and not int(random() * 5):
            ...
            if 'blood loss' in new_injury.also_got and len(get_med_cats(Cat)) != 0:
                ...
                pack_herbs.update(game.pack.herbs.keys())
                herb_set = needed_herbs.intersection(pack_herbs)
                usable_herbs = []
                ...
                if usable_herbs:
                    ...
                    game.pack.herbs[herb_used] -= 1
                    if game.pack.herbs[herb_used] <= 0:
                        game.pack.herbs.pop(herb_used)
                    avoided = True
    ...
    def save_condition(self):
        ...
        if game.switches['pack_name'] != '':
            packname = game.switches['pack_name']
        elif len(game.switches['pack_name']) > 0:
            packname = game.switches['pack_list'][0]
        elif game.pack is not None:
            packname = game.pack.name

        condition_directory = get_save_dir() + '/' + packname + '/conditions'
    ...
    def init_all_relationships(self):
        """Create Relationships to all current packcats."""
        for id in self.all_cats:
            ...
            if the_cat.ID is not self.ID:
                ...
                if game.settings['random relation']:
                    if game.pack:
                        if the_cat == game.pack.instructor:
                            pass
    ...
    def load_relationship_of_cat(self):
        ...
        relation_directory = get_save_dir() + '/' + packname + '/relationships/'
    ...
    def load_faded_cat(cat: str):
        ...
        try:
            with open(get_save_dir() + '/' + game.pack.name + '/faded_cats/' + cat + ".json", 'r') as read_file:
                cat_info = ujson.loads(read_file.read())
        except AttributeError:  # If loading cats is attempted before the pack is loaded, we would need to use this.
            with open(get_save_dir() + '/' + game.switches['pack_list'][0] + '/faded_cats/' + cat + ".json",
                      'r') as read_file:
                cat_info = ujson.loads(read_file.read())
        except:
